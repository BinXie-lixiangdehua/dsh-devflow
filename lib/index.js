...
const DEVFLOW_PRESET_ID$1 = "devflow";
...
//#endregion
//#region lib/host/activation-session-hook.js
/**
...
*/
function hostDrivesPerAgentActivation(service) {
	if (service === void 0 || service === null) return true;
	const probe = service;
...
*/
function resolveActivationMode(mode, presetService) {
...
*/
function createDevFlowSessionActivation(deps, mode = "auto") {
	...
	*/
	...
	const modeListeners = /* @__PURE__ */ new Set();
	...
	const onAgentCreated = (payload) => {
		...
		track(activateOne(agent, String(agent.id)));
	};
...
function resolveConfig(config) {
	...
	if (sessionActivation !== "auto" && sessionActivation !== "host" && sessionActivation !== "preset-row") throw new Error(`DevFlowConfig.sessionActivation must be "auto", "host" or "preset-row"; got ${JSON.stringify(sessionActivation)}`);
	const unknown = Object.keys(config).filter((key) => key !== "devflowDir" && key !== "stateDir" && key !== "sessionActivation");
	if (unknown.length > 0) throw new Error(`DevFlowConfig has unknown key(s) ${unknown.join(", ")}; config is { devflowDir?, stateDir?, sessionActivation? }`);
	return {
		devflowDir,
		sessionActivation
	};
	...
	*/
	...
	constructor(ctx, config = {}) {
		...
		createDevFlowSessionActivation({
			...
			warn: (message) => {
				ctx.logger("devflow").warn(message);
			}
