...
function locationsOf(token) {
	...
	return [cut <= 0 ? "" : trimmed.slice(0, cut), trimmed].filter((location) => location !== "").map((location) => location.toLowerCase());
}
/**
...
function mentionedLocationsOf(text) {
	const normalized = text.replace(/\\/g, "/");
	const out = /* @__PURE__ */ new Set();
	for (const token of normalized.match(PATH_TOKEN) ?? []) for (const location of locationsOf(token)) out.add(location);
	return out;
}
...
function assertPackageScopeConsistency(task, scope) {
	...
	if (namedLocations.size > 0) {
		...
		if ([...namedLocations].every((location) => !mentionedLocations.has(location))) throw new DispatchScopeConflict("the bounds name a location this task does not describe");
	}
...
function blockedRows(state, agentInstances) {
	return Object.values(state.blockedReports ?? {}).map((item) => {
