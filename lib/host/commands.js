...
import { BUILTIN_AGENT_ROLES } from "./protocol.js";
import { ROLE_DISPLAY, displayRole } from "./display.js";
import { recordFixedRosterChanges } from "./default-agents.js";
import { deriveNewProjectIdentity } from "./session-store.js";
import { AgentWorkflow as AgentWorkflowClass } from "./workflow-agent.js";
import { TaskWorkflow } from "./workflow.js";
import { ACTIVATION_PHASE_REASONS_ZH } from "./activation-reason.js";
 ...
 */
export function registerDevFlowCommands(ctx, services) {
     ...
     */
    const storeFor = (agent) => sessionStores === undefined ? store : sessionStores.resolve(agent).store;
    /**
     ...
     */
    const commandWorkflows = new Map();
            ...
            handler: async ({ agent, rawInput }) => {
                ...
                switch (verb) {
                    case 'init': {
                        ...
                        if (input === '')
                            return { kind: 'error', text: 'Usage: /devflow init <name> — a project name is required to initialize DevFlow.' };
                    ...
                    case 'commander': {
                        ...
                        if (action === 'enter') {
                            ...
                            if (existingProject !== undefined)
                                await hydrateProjectJournal(project, sessionStore);
                            await syncFixedRoster(sessionStore);
                        ...
                        if (action === 'say') {
                            ...
                            if (input === '')
                                return { kind: 'error', text: '/devflow commander say is no longer a command; send the message as normal Chat input.' };
                    ...
                    case 'decision-answer': {
                        ...
                        if (requestId === '' || rawAnswer === '')
                            return { kind: 'error', text: '/devflow decision-answer needs a request id and answer.' };
                        let answer;
                        try {
                            ...
                            if ('optionId' in parsed && typeof parsed.optionId === 'string')
                                answer = { optionId: parsed.optionId };
                            else if ('custom' in parsed && typeof parsed.custom === 'string')
                                answer = { custom: parsed.custom };
                            else
                                throw new Error('optionId or custom required');
                        }
                        catch {
                    ...
                    case 'agent-config': {
                        ...
                        if (agentId === '' || rawPatch === '')
                            return { kind: 'error', text: '/devflow agent-config needs an agent id and JSON patch.' };
                        let patch;
                        try {
                            patch = JSON.parse(rawPatch);
                        }
                        catch {
                            return { kind: 'error', text: '/devflow agent-config received invalid JSON.' };
                        }
                        if (typeof patch !== 'object' || patch === null || Array.isArray(patch))
                            return { kind: 'error', text: '/devflow agent-config needs a JSON object.' };
                    ...
                    case 'status': {
                        ...
                        if (project === undefined)
                            return { kind: 'success', text: 'No project initialized.' };
                        return {
                            kind: 'success',
                    ...
                    case 'tasks': {
                        ...
                        if (tasks.length === 0)
                            return { kind: 'success', text: `项目 ${sessionStore.rootPath}（本会话工作区）：No tasks.` };
                    ...
                    case 'show': {
                        ...
                        if (taskId === '')
                            return { kind: 'error', text: '/devflow show needs a task id.' };
                        const task = await sessionStore.getTask(taskId);
                        if (task === undefined)
                            return { kind: 'error', text: `Unknown task ${taskId}.` };
                    ...
                    case 'export': {
                        const taskId = (rest[0] ?? '').trim();
                        if (taskId === '')
                            return { kind: 'error', text: '/devflow export needs a task id.' };
                    ...
                    case 'import': {
                        const taskId = (rest[0] ?? '').trim();
                        if (taskId === '')
                            return { kind: 'error', text: '/devflow import needs a task id.' };
                    ...
                    case 'resume': {
                        const taskId = (rest[0] ?? '').trim();
                        if (taskId === '')
                            return { kind: 'error', text: '/devflow resume needs a task id.' };
