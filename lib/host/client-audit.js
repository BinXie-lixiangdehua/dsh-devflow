...
import { DEVFLOW_CLIENT_AUDIT_PAGE_BYTES, DEVFLOW_CLIENT_AUDIT_PAGE_LIMIT, DEVFLOW_CLIENT_AUDIT_PAGE_VERSION, DEVFLOW_CLIENT_AUDIT_TEXT_LIMIT, } from "../contract.js";
...
export class DevFlowAuditPager {
    ...
    async page(query = {}) {
        try {
            ...
            if (query.cursor !== undefined && cursor === undefined)
                return { kind: 'error', error: CURSOR_INVALID };
            const normalized = normalizeQuery(query, cursor);
            if (normalized === undefined)
                return { kind: 'error', error: CURSOR_INVALID };
            if (cursor !== undefined && cursor.projectId !== (project?.id ?? null))
                return { kind: 'error', error: CURSOR_INVALID };
            const scan = await store.readCommittedJournalSequencePage(cursor?.head, cursor?.next, DEVFLOW_AUDIT_CANDIDATE_SCAN_LIMIT);
    ...
    buildPage(projectId, query, entries, capturedHeadSequence, scanNext) {
        ...
        for (const entry of entries) {
            ...
            if (!inRange(item.at, query) || !matchesFilter(item, query.filter))
                continue;
        ...
        if (serializedBytes(page) > DEVFLOW_CLIENT_AUDIT_PAGE_BYTES)
            throw new Error('audit page exceeds response budget');
        return page;
    }
...
export function toSafeAuditItem(entry) {
    if (!validId(entry.id) || !Number.isSafeInteger(entry.sequence) || entry.sequence < 0 || !validTime(entry.at))
        return undefined;
    ...
    const id = (key) => stringField(data, key);
...
function normalizeQuery(query, cursor) {
    ...
    if (!validFilter(filter) || (cursor !== undefined && !sameFilter(filter, cursor.filter)))
        return undefined;
    const now = new Date();
    ...
    if (!validTime(fromValue) || !validTime(toValue) || (cursor !== undefined && (fromValue !== cursor.from || toValue !== cursor.to)))
        return undefined;
    const from = Date.parse(fromValue);
    const to = Date.parse(toValue);
    if (from > to || to - from > DEVFLOW_AUDIT_MAX_RANGE_MS)
        return undefined;
...
function matchesFilter(item, filter) {
    ...
    if (filter.kind === 'phase')
        return item.entity.type === 'phase' && item.entity.id === id || item.related.phaseId === id;
    if (filter.kind === 'task')
        return item.entity.type === 'task' && item.entity.id === id || item.related.taskId === id;
    if (filter.kind === 'agent')
        return item.entity.type === 'agent' && item.entity.id === id || item.related.agentId === id;
    if (filter.kind === 'execution')
        return item.entity.type === 'execution' && item.entity.id === id || item.related.executionId === id;
...
function safeText(value) {
    if (SENSITIVE_TEXT.test(value))
        return { text: REDACTED_TEXT, truncated: false, redacted: true };
    if (value.length <= DEVFLOW_CLIENT_AUDIT_TEXT_LIMIT)
        return { text: value, truncated: false, redacted: false };
...
function isCursorPayload(value) {
    ...
    return candidate.v === 1 && (candidate.projectId === null || typeof candidate.projectId === 'string')
        && isFilter(candidate.filter) && validTime(candidate.from) && validTime(candidate.to)
