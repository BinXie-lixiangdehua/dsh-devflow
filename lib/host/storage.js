...
import { applyDevFlowStateEvent } from "./state.js";
import { recordDevFlowChange } from "./journal.js";
import { isRecord, isString, isStringArray } from "./json.js";
import { isDevFlowCloseReason } from "./types.js";
import { DEVFLOW_SKILLS, validateAgentSkills } from "./skill-binding.js";
...
const TEMPORARY_STATUSES = ['created', 'running', 'terminated'];
...
const ASSIGNMENT_STATUSES = ['assigned', 'in_progress', 'completed', 'closed'];
/**
...
const COMMANDER_PLAN_STATUSES = ['draft', 'active', 'completed'];
const EXECUTION_BATCH_STATUSES = ['planned', 'running', 'paused', 'completed'];
...
const EXECUTION_STATUSES = ['pending', 'running', 'completed', 'failed', 'closed'];
/**
...
const AGENT_REPORT_STATUSES = ['success', 'failed', 'blocked'];
const DECISIONS_DIR = 'decisions';
const DECISION_TYPES = ['continue', 'retry', 'pause', 'request_user'];
const ACTIONS_DIR = 'actions';
const ACTION_TYPES = ['start_batch', 'pause_batch', 'retry_execution', 'complete_batch'];
const ACTION_STATUSES = ['created', 'executing', 'completed'];
...
const RUNTIME_SESSION_STATUSES = ['created', 'running', 'completed', 'failed'];
...
const ATTEMPT_STATUSES = ['created', 'running', 'completed', 'failed'];
...
const MEMORY_TYPES = ['project', 'decision', 'execution', 'preference'];
...
const ACTION_EXECUTION_STATUSES = ['running', 'completed', 'failed'];
...
const PROPOSAL_STATUSES = ['created', 'approved', 'rejected'];
...
const WORKFLOW_STATUSES = ['created', 'running', 'completed', 'failed'];
...
function validateProject(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid project record in ${displayPath}`);
...
function validateTask(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid task record in ${displayPath}`);
    ...
    if (assignedRole !== undefined) {
        if (!ASSIGNED_ROLES.includes(assignedRole)) {
    ...
    return {
        ...
        description,
        status: status,
        ...(assignedRole === undefined ? {} : { assignedRole: assignedRole }),
        createdAt,
        updatedAt,
    };
}
...
function validateResult(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid result record in ${displayPath}`);
...
function validateAgentInstance(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid agent instance record in ${displayPath}`);
    ...
    if (!isString(role) || !ASSIGNED_ROLES.includes(role)) {
...
function validateModelConfig(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid agent modelConfig in ${displayPath}`);
...
function validateOrchestrationAgent(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid orchestration agent record in ${displayPath}`);
    ...
    if (!isString(kind) || !AGENT_KINDS.includes(kind)) {
    ...
    if (!isString(status) || !allowedStatuses.includes(status)) {
        throw new Error(`devflow: invalid orchestration agent status ${JSON.stringify(status)} in ${displayPath}`);
    }
    if (!isString(prompt))
        throw new Error(`devflow: invalid orchestration agent prompt in ${displayPath}`);
    if (!isStringArray(tools))
        throw new Error(`devflow: invalid orchestration agent tools in ${displayPath}`);
    if (!isStringArray(capabilities))
        throw new Error(`devflow: invalid orchestration agent capabilities in ${displayPath}`);
    if (!isStringArray(skills))
        throw new Error(`devflow: invalid orchestration agent skills in ${displayPath}`);
...
function validateMvpPlan(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid mvp plan record in ${displayPath}`);
...
function validatePhase(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid phase record in ${displayPath}`);
    ...
    return { id, name, description, status: status, createdAt, updatedAt };
}
/** Validate one scope guard record at the durable-file boundary. */
function validateScopeGuard(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid scope guard record in ${displayPath}`);
...
function validateImprovement(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid improvement record in ${displayPath}`);
...
function validateCommanderCheckpoint(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander checkpoint record in ${displayPath}`);
    ...
    if (!isString(checkpointId))
        throw new Error(`devflow: invalid commander checkpoint id in ${displayPath}`);
    if (!isString(projectId))
        throw new Error(`devflow: invalid commander checkpoint projectId in ${displayPath}`);
...
function validatePhaseAssignment(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid phase assignment record in ${displayPath}`);
    ...
    if (!isString(assignmentId))
        throw new Error(`devflow: invalid phase assignment id in ${displayPath}`);
    if (taskId !== undefined && (!isString(taskId) || taskId.trim() === '')) {
        throw new Error(`devflow: invalid phase assignment taskId in ${displayPath}`);
    }
    if (!isString(phaseId))
        throw new Error(`devflow: invalid phase assignment phaseId in ${displayPath}`);
    if (!isString(agentId) || !AGENT_ID_PATTERN.test(agentId)) {
    ...
    if (!isString(status) || !ASSIGNMENT_STATUSES.includes(status)) {
    ...
    return {
        ...
        role: role,
        status: status,
        ...(status === 'closed' ? { closedAt: closedAt, closeReason: closeReason } : {}),
        createdAt, updatedAt,
    };
}
...
function validateCommanderPlan(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander plan record in ${displayPath}`);
    const { planningId, projectId, goal, status, mvpPlanId, createdAt, updatedAt } = value;
    if (!isString(planningId))
        throw new Error(`devflow: invalid commander plan id in ${displayPath}`);
    if (!isString(projectId))
        throw new Error(`devflow: invalid commander plan projectId in ${displayPath}`);
    if (!isString(goal))
        throw new Error(`devflow: invalid commander plan goal in ${displayPath}`);
    if (!isString(status) || !COMMANDER_PLAN_STATUSES.includes(status)) {
...
function validateExecutionBatch(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid execution batch record in ${displayPath}`);
    const { batchId, projectId, planningId, phaseIds, assignmentIds, status, createdAt, updatedAt } = value;
    if (!isString(batchId))
        throw new Error(`devflow: invalid execution batch id in ${displayPath}`);
    if (!isString(projectId))
        throw new Error(`devflow: invalid execution batch projectId in ${displayPath}`);
    if (!isString(planningId))
        throw new Error(`devflow: invalid execution batch planningId in ${displayPath}`);
    if (!isStringArray(phaseIds))
        throw new Error(`devflow: invalid execution batch phaseIds in ${displayPath}`);
    if (!isStringArray(assignmentIds))
        throw new Error(`devflow: invalid execution batch assignmentIds in ${displayPath}`);
    if (!isString(status) || !EXECUTION_BATCH_STATUSES.includes(status)) {
...
function validateTaskScopeGuard(value, displayPath) {
    if (value === undefined)
        return undefined;
    if (!isRecord(value))
        throw new Error(`devflow: invalid task scope guard in ${displayPath}`);
...
function validateExecutionRecord(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid execution record in ${displayPath}`);
    ...
    if (!isString(executionId))
        throw new Error(`devflow: invalid execution id in ${displayPath}`);
    if (!isString(batchId))
        throw new Error(`devflow: invalid execution batchId in ${displayPath}`);
    if (!isString(assignmentId))
        throw new Error(`devflow: invalid execution assignmentId in ${displayPath}`);
    if (!isString(agentId) || !AGENT_ID_PATTERN.test(agentId)) {
    ...
    if (!isString(status) || !EXECUTION_STATUSES.includes(status)) {
    ...
    if (startedAt !== null && !isString(startedAt))
        throw new Error(`devflow: invalid execution startedAt in ${displayPath}`);
    if (completedAt !== null && !isString(completedAt))
        throw new Error(`devflow: invalid execution completedAt in ${displayPath}`);
    // The close stamp is EXACTLY the `closed` state: both fields together or neither.
    if (status === 'closed') {
        if (!isString(closedAt) || !isDevFlowCloseReason(closeReason)) {
    ...
    if (!isString(createdAt) || !isString(updatedAt))
        throw new Error(`devflow: invalid execution timestamps in ${displayPath}`);
    return {
...
function validateAgentReport(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid agent report record in ${displayPath}`);
    ...
    if (!isString(reportId))
        throw new Error(`devflow: invalid agent report id in ${displayPath}`);
    if (!isString(executionId))
        throw new Error(`devflow: invalid agent report executionId in ${displayPath}`);
    if (!isString(agentId) || !AGENT_ID_PATTERN.test(agentId)) {
        throw new Error(`devflow: invalid agent report agentId in ${displayPath}`);
    }
    if (!isString(status) || !AGENT_REPORT_STATUSES.includes(status)) {
        throw new Error(`devflow: invalid agent report status ${JSON.stringify(status)} in ${displayPath}`);
    }
    if (!isString(summary))
        throw new Error(`devflow: invalid agent report summary in ${displayPath}`);
    if (!isString(outputReference))
        throw new Error(`devflow: invalid agent report outputReference in ${displayPath}`);
    ...
    if (!isString(createdAt) || !isString(updatedAt))
        throw new Error(`devflow: invalid agent report timestamps in ${displayPath}`);
    return {
...
function validateCommanderDecision(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander decision record in ${displayPath}`);
    const { decisionId, projectId, checkpointId, relatedExecutionIds, decisionType, summary, nextAction, createdAt, updatedAt, } = value;
    if (!isString(decisionId))
        throw new Error(`devflow: invalid commander decision id in ${displayPath}`);
    if (!isString(projectId))
        throw new Error(`devflow: invalid commander decision projectId in ${displayPath}`);
    ...
    if (!isString(decisionType) || !DECISION_TYPES.includes(decisionType)) {
        throw new Error(`devflow: invalid commander decision type ${JSON.stringify(decisionType)} in ${displayPath}`);
    }
    if (!isString(summary))
        throw new Error(`devflow: invalid commander decision summary in ${displayPath}`);
    if (!isString(nextAction))
        throw new Error(`devflow: invalid commander decision nextAction in ${displayPath}`);
    if (!isString(createdAt) || !isString(updatedAt)) {
...
function validateCommanderAction(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander action record in ${displayPath}`);
    const { actionId, decisionId, actionType, targetId, status, createdAt, updatedAt } = value;
    if (!isString(actionId))
        throw new Error(`devflow: invalid commander action id in ${displayPath}`);
    if (!isString(decisionId))
        throw new Error(`devflow: invalid commander action decisionId in ${displayPath}`);
    if (!isString(actionType) || !ACTION_TYPES.includes(actionType)) {
    ...
    if (!isString(status) || !ACTION_STATUSES.includes(status)) {
...
function validateRuntimeSession(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid runtime session record in ${displayPath}`);
    const { sessionId, executionId, agentId, status, startedAt, completedAt, metadata, createdAt, updatedAt } = value;
    if (!isString(sessionId))
        throw new Error(`devflow: invalid runtime session id in ${displayPath}`);
    if (!isString(executionId))
        throw new Error(`devflow: invalid runtime session executionId in ${displayPath}`);
    if (!isString(agentId) || !AGENT_ID_PATTERN.test(agentId)) {
        throw new Error(`devflow: invalid runtime session agentId in ${displayPath}`);
    }
    if (!isString(status) || !RUNTIME_SESSION_STATUSES.includes(status)) {
    ...
    if (!isRecord(metadata))
        throw new Error(`devflow: invalid runtime session metadata in ${displayPath}`);
    if (!isString(createdAt) || !isString(updatedAt)) {
...
function validateExecutionAttempt(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid execution attempt record in ${displayPath}`);
    const { attemptId, executionId, parentAttemptId, status, reason, createdAt, completedAt, updatedAt } = value;
    if (!isString(attemptId))
        throw new Error(`devflow: invalid execution attempt id in ${displayPath}`);
    if (!isString(executionId))
        throw new Error(`devflow: invalid execution attempt executionId in ${displayPath}`);
    ...
    if (!isString(status) || !ATTEMPT_STATUSES.includes(status)) {
    ...
    if (!isString(createdAt))
        throw new Error(`devflow: invalid execution attempt createdAt in ${displayPath}`);
    if (completedAt !== null && !isString(completedAt)) {
        throw new Error(`devflow: invalid execution attempt completedAt in ${displayPath}`);
    }
    if (!isString(updatedAt))
        throw new Error(`devflow: invalid execution attempt updatedAt in ${displayPath}`);
    return {
...
function validateCommanderReview(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander review record in ${displayPath}`);
    const { reviewId, projectId, reportId, executionId, status, summary, createdAt, updatedAt } = value;
    if (!isString(reviewId))
        throw new Error(`devflow: invalid commander review id in ${displayPath}`);
    if (!isString(projectId))
        throw new Error(`devflow: invalid commander review projectId in ${displayPath}`);
    if (!isString(reportId))
        throw new Error(`devflow: invalid commander review reportId in ${displayPath}`);
    if (!isString(executionId))
        throw new Error(`devflow: invalid commander review executionId in ${displayPath}`);
    if (!isString(status) || !REVIEW_STATUSES.includes(status)) {
        throw new Error(`devflow: invalid commander review status ${JSON.stringify(status)} in ${displayPath}`);
    }
    if (!isString(summary))
        throw new Error(`devflow: invalid commander review summary in ${displayPath}`);
    if (!isString(createdAt) || !isString(updatedAt)) {
...
function validateCommanderMemory(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander memory record in ${displayPath}`);
    const { memoryId, projectId, memoryType, content, source, createdAt, updatedAt } = value;
    if (!isString(memoryId))
        throw new Error(`devflow: invalid commander memory id in ${displayPath}`);
    if (!isString(projectId))
        throw new Error(`devflow: invalid commander memory projectId in ${displayPath}`);
    if (!isString(memoryType) || !MEMORY_TYPES.includes(memoryType)) {
...
function validateCommanderSchedule(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander schedule record in ${displayPath}`);
    const { scheduleId, projectId, status, interval, lastRunAt, nextRunAt, createdAt, updatedAt } = value;
    if (!isString(scheduleId))
        throw new Error(`devflow: invalid commander schedule id in ${displayPath}`);
    if (!isString(projectId))
        throw new Error(`devflow: invalid commander schedule projectId in ${displayPath}`);
    if (!isString(status) || !SCHEDULE_STATUSES.includes(status)) {
    ...
    if (!isString(nextRunAt))
        throw new Error(`devflow: invalid commander schedule nextRunAt in ${displayPath}`);
    if (!isString(createdAt) || !isString(updatedAt)) {
...
function validateCommanderRunRecord(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander run record in ${displayPath}`);
    const { runId, projectId, scheduleId, status, decisionId, actionId, startedAt, completedAt, updatedAt } = value;
    if (!isString(runId))
        throw new Error(`devflow: invalid commander run id in ${displayPath}`);
    if (!isString(projectId))
        throw new Error(`devflow: invalid commander run projectId in ${displayPath}`);
    if (!isString(scheduleId))
        throw new Error(`devflow: invalid commander run scheduleId in ${displayPath}`);
    if (!isString(status) || !RUN_STATUSES.includes(status)) {
    ...
    if (!isString(startedAt))
        throw new Error(`devflow: invalid commander run startedAt in ${displayPath}`);
    if (completedAt !== null && !isString(completedAt)) {
        throw new Error(`devflow: invalid commander run completedAt in ${displayPath}`);
    }
    if (!isString(updatedAt))
        throw new Error(`devflow: invalid commander run updatedAt in ${displayPath}`);
    return {
...
function validateCommanderActionExecution(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander action execution record in ${displayPath}`);
    const { executionId, actionId, status, success, error, createdAt, completedAt, updatedAt } = value;
    if (!isString(executionId))
        throw new Error(`devflow: invalid commander action execution id in ${displayPath}`);
    if (!isString(actionId))
        throw new Error(`devflow: invalid commander action execution actionId in ${displayPath}`);
    if (!isString(status) || !ACTION_EXECUTION_STATUSES.includes(status)) {
    ...
    if (!isString(createdAt))
        throw new Error(`devflow: invalid commander action execution createdAt in ${displayPath}`);
    if (completedAt !== null && !isString(completedAt)) {
        throw new Error(`devflow: invalid commander action execution completedAt in ${displayPath}`);
    }
    if (!isString(updatedAt))
        throw new Error(`devflow: invalid commander action execution updatedAt in ${displayPath}`);
    return {
...
function validateCommanderPolicy(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander policy record in ${displayPath}`);
    ...
    if (!isStringArray(allowedActionTypes) || !allowedActionTypes.every(type => ACTION_TYPES.includes(type))) {
    ...
    if (!isStringArray(requireApprovalActionTypes)
        || !requireApprovalActionTypes.every(type => ACTION_TYPES.includes(type))) {
        throw new Error(`devflow: invalid commander policy requireApprovalActionTypes in ${displayPath}`);
    }
    if (!isString(riskLevel) || !RISK_LEVELS.includes(riskLevel)) {
    ...
    return {
        ...
        requireApprovalActionTypes: requireApprovalActionTypes,
        riskLevel: riskLevel, createdAt, updatedAt,
    };
}
/** Validate one commander proposal record at the durable-file boundary. */
function validateCommanderProposal(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander proposal record in ${displayPath}`);
    const { proposalId, decisionId, actionType, targetId, riskLevel, status, createdAt, updatedAt } = value;
    if (!isString(proposalId))
        throw new Error(`devflow: invalid commander proposal id in ${displayPath}`);
    if (!isString(decisionId))
        throw new Error(`devflow: invalid commander proposal decisionId in ${displayPath}`);
    if (!isString(actionType) || !ACTION_TYPES.includes(actionType)) {
    ...
    if (!isString(status) || !PROPOSAL_STATUSES.includes(status)) {
...
function validateCommanderWorkflowExecution(value, displayPath) {
    ...
    if (!isString(createdAt))
        throw new Error(`devflow: invalid workflow execution createdAt in ${displayPath}`);
    return {
...
function validateCommanderWorkflow(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander workflow record in ${displayPath}`);
    const { workflowId, projectId, title, description, status, history, createdAt, updatedAt } = value;
    if (!isString(workflowId))
        throw new Error(`devflow: invalid commander workflow id in ${displayPath}`);
    if (!isString(projectId))
        throw new Error(`devflow: invalid commander workflow projectId in ${displayPath}`);
    if (!isString(title) || title.trim() === '') {
        throw new Error(`devflow: invalid commander workflow title in ${displayPath}`);
    }
    if (!isString(description))
        throw new Error(`devflow: invalid commander workflow description in ${displayPath}`);
    if (!isString(status) || !WORKFLOW_STATUSES.includes(status)) {
        throw new Error(`devflow: invalid commander workflow status ${JSON.stringify(status)} in ${displayPath}`);
    }
    if (!Array.isArray(history))
        throw new Error(`devflow: invalid commander workflow history in ${displayPath}`);
...
function validateCommanderWorkflowStep(value, displayPath) {
    if (!isRecord(value))
        throw new Error(`devflow: invalid commander workflow step record in ${displayPath}`);
    const { stepId, workflowId, stepIndex, title, status, createdAt, updatedAt } = value;
    if (!isString(stepId))
        throw new Error(`devflow: invalid commander workflow step id in ${displayPath}`);
    if (!isString(workflowId))
        throw new Error(`devflow: invalid commander workflow step workflowId in ${displayPath}`);
    ...
    if (!isString(status) || !STEP_STATUSES.includes(status)) {
...
export class DevFlowStore {
    ...
    async writeJson(target, value, expected, options) {
        ...
        await this.fs.writeText(target, `${JSON.stringify(value, null, 2)}\n`, expected, undefined, this.sandboxPolicyOf?.());
        if (this.onChange === undefined || options?.report === false)
            return;
        ...
        if (options?.carryPendingRecord === true)
            this.pendingJournalRecord = null;
        try {
            if (sequence === undefined && record === undefined)
                this.onChange(relative);
            else
                this.onChange(relative, sequence, record);
        }
    ...
    async appendJournal(type, data, id = randomUUID()) {
        ...
        try {
            ...
            release();
            if (journalTails.get(journalRoot) === tail)
                journalTails.delete(journalRoot);
        }
    }
    ...
    async appendJournalUnlocked(type, data, id) {
        if (!isString(type) || type.trim() === '')
            throw new Error('devflow: journal type must be a non-empty string');
        if (!isJsonValue(data))
            throw new Error(`devflow: journal ${type} carries non-JSON data`);
        for (;;) {
            ...
            if (existing !== undefined) {
                ...
                try {
                    await this.writeJson(headTarget, { nextSequence: prior.sequence + 1 }, headInfo === undefined ? { kind: 'createIfAbsent' } : { kind: 'replaceIfVersion', version: headInfo.version });
                }
                catch (error) {
                    if (headInfo === undefined || error.code === 'FS_STALE_VERSION')
                        continue;
                    throw error;
                }
                continue;
            ...
            try {
                await this.writeJson(headTarget, { nextSequence: entry.sequence + 1 }, headInfo === undefined ? { kind: 'createIfAbsent' } : { kind: 'replaceIfVersion', version: headInfo.version }, { carryPendingRecord: true });
    ...
    async readCommittedJournalSequencePage(capturedHeadSequence, nextExclusiveSequence, scanLimit) {
        ...
        while (cursor > 0 && scannedCount < scanLimit) {
            ...
            if (raw === undefined)
                throw new Error('devflow: committed journal entry is unavailable');
            const entry = validateJournalEntry(raw, target.displayPath);
            if (entry.sequence !== sequence)
                throw new Error('devflow: committed journal sequence mismatch');
            entries.push(entry);
            scannedCount++;
            cursor = sequence;
        }
    ...
    async listJournal() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            if (entry.type !== 'file' || !entry.name.endsWith(FILE_SUFFIX) || entry.name === JOURNAL_HEAD_FILE)
                continue;
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                entries.push(validateJournalEntry(raw, entry.target.displayPath));
        }
        const headTarget = await this.journalHeadTarget();
        const headRaw = await this.readJson(headTarget);
    ...
    async listTasks() {
        ...
        const tasks = [];
        for (const entry of await this.fs.listDir(dir)) {
            if (entry.type !== 'file' || !entry.name.endsWith(FILE_SUFFIX))
                continue;
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                tasks.push(validateTask(raw, entry.target.displayPath));
        }
    ...
    async saveResult(input) {
        const result = { ...input, id: randomUUID(), createdAt: new Date().toISOString() };
    ...
    async listResults() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                results.push(validateResult(raw, entry.target.displayPath));
        }
    ...
    async listAgentInstances() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                instances.push(validateAgentInstance(raw, entry.target.displayPath));
        }
    ...
    async registerAgent(input) {
        ...
        const agent = { ...input, status, createdAt: now, updatedAt: now };
    ...
    async resolveAgentSkills(agent) {
        ...
        for (const skillId of agent.skills) {
            ...
            if (definition === undefined)
                throw new Error(`devflow: unknown skill id ${JSON.stringify(skillId)}`);
    ...
    async listAgents() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined && !isTombstone(raw))
                agents.push(validateOrchestrationAgent(raw, entry.target.displayPath));
        }
    ...
    async removeAgent(agentId) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot remove unknown agent ${agentId}`);
    ...
    async updateAgentConfig(agentId, patch) {
        ...
        const existing = await this.getAgent(agentId);
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown agent ${agentId}`);
    ...
    async createPhase(input) {
        const now = new Date().toISOString();
        const phase = { ...input, id: randomUUID(), createdAt: now, updatedAt: now };
    ...
    async updatePhaseStatus(id, status) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown phase ${id}`);
    ...
    async updateScope(scope) {
        ...
        const now = new Date().toISOString();
        const updated = { ...scope, createdAt: existing?.createdAt ?? now, updatedAt: now };
    ...
    async addImprovement(input) {
        const improvement = { ...input, id: randomUUID(), createdAt: new Date().toISOString() };
    ...
    async listImprovements() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                improvements.push(validateImprovement(raw, entry.target.displayPath));
        }
    ...
    async createCheckpoint(input) {
        const now = new Date().toISOString();
        const checkpoint = { ...input, checkpointId: randomUUID(), createdAt: now, updatedAt: now };
    ...
    async updateCheckpoint(id, patch) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown checkpoint ${id}`);
    ...
    async listCheckpoints() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                checkpoints.push(validateCommanderCheckpoint(raw, entry.target.displayPath));
        }
    ...
    async createAssignment(input) {
        const now = new Date().toISOString();
        const assignment = { ...input, assignmentId: randomUUID(), createdAt: now, updatedAt: now };
    ...
    async updateAssignmentStatus(id, status, closeReason) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown assignment ${id}`);
        ...
        const updated = {
            ...rest,
            status,
            ...(status === 'closed' ? { closedAt: now, closeReason: closeReason } : {}),
            updatedAt: now,
        };
    ...
    async unassignAssignment(id) {
        ...
        const existing = await this.getAssignment(id);
        if (existing === undefined)
            throw new Error(`devflow: cannot unassign unknown assignment ${id}`);
    ...
    async listAssignments() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined && !isTombstone(raw))
                assignments.push(validatePhaseAssignment(raw, entry.target.displayPath));
        }
    ...
    async createPlanning(input) {
        const now = new Date().toISOString();
        const plan = { ...input, planningId: randomUUID(), status: 'draft', createdAt: now, updatedAt: now };
    ...
    async updatePlanning(id, patch) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown plan ${id}`);
    ...
    async activatePlanning(id) {
        ...
        const existing = await this.getPlanning(id);
        if (existing === undefined)
            throw new Error(`devflow: cannot activate unknown plan ${id}`);
        if (existing.status !== 'draft') {
            throw new Error(`devflow: cannot activate plan ${id} in status ${existing.status}`);
        }
        const updated = { ...existing, status: 'active', updatedAt: new Date().toISOString() };
        validateCommanderPlan(updated, 'memory');
        await this.writeJson(await this.planningTarget(id), updated);
        return updated;
    }
    ...
    async listPlanning() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                plans.push(validateCommanderPlan(raw, entry.target.displayPath));
        }
    ...
    async createBatch(input) {
        const now = new Date().toISOString();
        const batch = { ...input, batchId: randomUUID(), status: 'planned', createdAt: now, updatedAt: now };
    ...
    async updateBatchStatus(id, status) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown batch ${id}`);
    ...
    async listBatches() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                batches.push(validateExecutionBatch(raw, entry.target.displayPath));
        }
    ...
    async updateExecutionStatus(id, status, closeReason) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown execution ${id}`);
    ...
    async listExecutionsByTask(taskId) {
        if (!isString(taskId) || taskId.trim() === '')
            throw new Error('devflow: taskId must be a non-empty string');
    ...
    async createReport(input) {
        const now = new Date().toISOString();
        const report = { ...input, reportId: randomUUID(), createdAt: now, updatedAt: now };
    ...
    async listReportsByTask(taskId) {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            if (raw !== undefined) {
                ...
                if (executionIds.has(report.executionId))
                    reports.push(report);
            }
        }
    ...
    async updateReport(id, patch) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown report ${id}`);
    ...
    async createDecision(input) {
        const now = new Date().toISOString();
        const decision = { ...input, decisionId: randomUUID(), createdAt: now, updatedAt: now };
    ...
    async listDecisions() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                decisions.push(validateCommanderDecision(raw, entry.target.displayPath));
        }
    ...
    async updateDecision(id, patch) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown decision ${id}`);
    ...
    async listActions() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                actions.push(validateCommanderAction(raw, entry.target.displayPath));
        }
    ...
    async updateActionStatus(id, status) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown action ${id}`);
    ...
    async updateRuntimeSessionStatus(id, status) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown runtime session ${id}`);
    ...
    async updateAttemptStatus(id, status) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown execution attempt ${id}`);
    ...
    async completeReview(id) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot complete unknown review ${id}`);
        ...
        const updated = { ...existing, status: 'reviewed', updatedAt: new Date().toISOString() };
    ...
    async createMemory(input) {
        const now = new Date().toISOString();
        const memory = { ...input, memoryId: randomUUID(), createdAt: now, updatedAt: now };
    ...
    async updateMemory(id, patch) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown memory ${id}`);
    ...
    async updateSchedule(id, patch) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot update unknown schedule ${id}`);
    ...
    async pauseSchedule(id) {
        ...
        const existing = await this.getSchedule(id);
        if (existing === undefined)
            throw new Error(`devflow: cannot pause unknown schedule ${id}`);
    ...
    async listSchedules() {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                schedules.push(validateCommanderSchedule(raw, entry.target.displayPath));
        }
    ...
    async completeRunRecord(id, decisionId, actionId) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot complete unknown run ${id}`);
    ...
    async failRunRecord(id) {
        ...
        const existing = await this.getRunRecord(id);
        if (existing === undefined)
            throw new Error(`devflow: cannot fail unknown run ${id}`);
        ...
        const now = new Date().toISOString();
        const updated = { ...existing, status: 'failed', completedAt: now, updatedAt: now };
        validateCommanderRunRecord(updated, 'memory');
        await this.writeJson(await this.runTarget(id), updated);
        return updated;
    }
    ...
    async completeActionExecution(id) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot complete unknown action execution ${id}`);
    ...
    async failActionExecution(id, error) {
        ...
        const existing = await this.getActionExecution(id);
        if (existing === undefined)
            throw new Error(`devflow: cannot fail unknown action execution ${id}`);
    ...
    async savePolicy(policy) {
        ...
        const now = new Date().toISOString();
        const saved = { ...policy, createdAt: existing?.createdAt ?? now, updatedAt: now };
    ...
    async approveProposal(id) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot approve unknown proposal ${id}`);
    ...
    async rejectProposal(id) {
        ...
        const existing = await this.getProposal(id);
        if (existing === undefined)
            throw new Error(`devflow: cannot reject unknown proposal ${id}`);
    ...
    async listProposalsByDecision(decisionId) {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            if (raw !== undefined) {
                ...
                if (proposal.decisionId === decisionId)
    ...
    async startWorkflow(id) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot start unknown workflow ${id}`);
        ...
        const updated = { ...existing, status: 'running', updatedAt: new Date().toISOString() };
    ...
    async completeWorkflow(id) {
        ...
        const existing = await this.getWorkflow(id);
        if (existing === undefined)
            throw new Error(`devflow: cannot complete unknown workflow ${id}`);
    ...
    async failWorkflow(id) {
        ...
        const existing = await this.getWorkflow(id);
        if (existing === undefined)
            throw new Error(`devflow: cannot fail unknown workflow ${id}`);
        ...
        const updated = { ...existing, status: 'failed', updatedAt: new Date().toISOString() };
        validateCommanderWorkflow(updated, 'memory');
        await this.writeJson(await this.workflowTarget(id), updated);
        return updated;
    }
    ...
    async appendWorkflowExecution(id, entry) {
        ...
        const existing = await this.getWorkflow(id);
        if (existing === undefined)
            throw new Error(`devflow: cannot append to unknown workflow ${id}`);
    ...
    async startStep(id) {
        ...
        if (existing === undefined)
            throw new Error(`devflow: cannot start unknown step ${id}`);
    ...
    async completeStep(id) {
        ...
        const existing = await this.getStep(id);
        if (existing === undefined)
            throw new Error(`devflow: cannot complete unknown step ${id}`);
    ...
    async failStep(id) {
        ...
        const existing = await this.getStep(id);
        if (existing === undefined)
            throw new Error(`devflow: cannot fail unknown step ${id}`);
    ...
    async listRecords(directory, validate) {
        ...
        for (const entry of await this.fs.listDir(dir)) {
            ...
            const raw = await this.readJson(entry.target);
            if (raw !== undefined)
                records.push(validate(raw, entry.target.displayPath));
        }
    ...
    async loadState() {
        ...
        for (const entry of journal) {
            if (!journalOnlyTypes.has(entry.type))
                continue;
