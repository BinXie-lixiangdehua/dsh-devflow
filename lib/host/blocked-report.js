 ...
 */
export function declaredOutcome(text) {
 ...
 */
export function classifyCapabilityGap(detail) {
    const text = detail.toLowerCase();
    if (/权限|授权|审批|提权|permission|not allowed|denied|forbidden/.test(text))
        return 'permission';
    if (TOOL_NAME.test(text) && /没有|缺少|缺|missing|no\s+(?:write|edit)|without|未挂载|不可用/.test(text))
        return 'tool';
    if (/依赖|dependency|not installed|未安装|找不到命令|command not found/.test(text))
        return 'dependency';
    return 'unstated';
}
...
function gapReason(detail) {
    if (!/(?:没有|缺少|缺|无|未挂载|不可用)/.test(detail))
        return detail;
...
function suggestedOwner(detail) {
    if (/boss|用户|你执行|需要你|请用户/.test(detail))
        return 'boss';
    if (/配置|preset|派发策略|宿主|DSH/.test(detail))
        return 'DevFlow 配置层';
    if (/总指挥/.test(detail))
        return '总指挥';
    return '未标明';
...
export function blockedHeadline(report, displayName) {
 ...
 */
export function capabilityBreaker(blocked) {
