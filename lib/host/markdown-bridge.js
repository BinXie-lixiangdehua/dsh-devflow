 ...
 */
import { assertSupportedProtocolVersion, validateTaskPackage } from "./bridge.js";
...
function extractResultDocument(content) {
    ...
    for (let index = headingIndex + 1; index < lines.length; index += 1) {
        ...
        if (heading !== undefined && !RESULT_SECTIONS.has(heading) && body.some(entry => entry.trim() !== ''))
            break;
        body.push(line);
    }
    return body;
...
function taskBoundarySection(pkg) {
    ...
    if (scope === undefined)
        return 'Project default applies';
    return [
 ...
 */
export function parseResultMarkdown(markdown) {
...
export function renderTaskMarkdown(pkg) {
