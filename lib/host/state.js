 ...
 */
import { activateCommanderPlan, answerDecisionRequest, appendCommanderWorkflowExecution, appendScopeBoundaryHit, approveCommanderProposal, completeCommanderActionExecution, completeCommanderRun, completeCommanderReview, completeCommanderWorkflow, completeCommanderWorkflowStep, failCommanderActionExecution, failCommanderRun, failCommanderWorkflow, failCommanderWorkflowStep, patchAgentReport, patchAssignmentStatus, patchCheckpoint, patchCommanderDecision, patchCommanderMemory, patchCommanderPlan, patchCommanderSchedule, patchOrchestrationAgent, pauseCommanderSchedule, rejectCommanderProposal, removeAssignment, removeOrchestrationAgent, startCommanderWorkflow, startCommanderWorkflowStep, transitionCommanderAction, transitionExecution, transitionExecutionAttempt, transitionExecutionBatch, transitionOrchestrationAgent, transitionRuntimeSession, upsertAgent, upsertAgentReport, upsertBlockedReport, upsertAssignment, upsertCheckpoint, upsertCommanderAction, upsertCommanderActionExecution, upsertCommanderDecision, upsertCommanderExecutionContext, upsertCommanderMemory, upsertCommanderPlan, upsertCommanderProposal, upsertCommanderReview, upsertCommanderRun, upsertCommanderSchedule, upsertCommanderWorkflow, upsertCommanderWorkflowStep, upsertExecution, upsertExecutionAttempt, upsertExecutionBatch, upsertOrchestrationAgent, upsertPhase, upsertPolicy, upsertDecisionRequest, upsertDispatchDiagnostic, upsertRuntimePackage, upsertRuntimeResult, upsertRuntimeSession, foldReviewFailCount, resetReviewFailCount, } from "./projection.js";
/** Create the empty durable DevFlow state. */
export function initialDevFlowState() {
    return {
        project: null,
        tasks: {},
        agents: [],
...
export function applyDevFlowStateEvent(state, event) {
    ...
    if (event.type === 'devflow/commander/mode-exit')
        return { ...state, commanderMode: 'chat', commanderModeExitAt: event.data.at };
    if (event.type === 'devflow/control/pause')
        return { ...state, paused: true };
    if (event.type === 'devflow/control/resume')
