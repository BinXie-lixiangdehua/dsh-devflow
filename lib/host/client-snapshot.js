...
import { DEVFLOW_CLIENT_SNAPSHOT_VERSION, } from "../contract.js";
import { fixedAgentDisplayName } from "./display.js";
import { blockedHeadline } from "./blocked-report.js";
...
export async function createDevFlowClientSnapshot(controller, agent) {
 ...
 */
export function blockedResolved(state, block) {
    // `state.tasks` is the taskId → status map (the full task objects live in `taskTitles`).
    if (state.tasks[block.taskId] === 'completed')
        return true;
    return Object.values(state.executions ?? {}).some(execution => execution.taskId === block.taskId
        && execution.status === 'completed'
        && (execution.completedAt ?? execution.updatedAt ?? execution.createdAt) > block.createdAt);
}
/**
...
function blockedRows(state, agentInstances) {
    return Object.values(state.blockedReports ?? {})
        .filter((item) => !blockedResolved(state, item))
        .map((item) => {
...
function createFailures(executions, reports) {
    const reportFailures = reports
        .filter((report) => report.status === 'failed' || report.status === 'blocked')
        .map(report => ({
        id: `report-${report.id}`,
        executionId: report.executionId,
...
function toSafeText(value) {
    ...
    if (value.length <= DEVFLOW_SAFE_TEXT_LIMIT)
