...
export class DevFlowSessionScopeError extends Error {
    ...
    constructor(sessionId, detail) {
        super(`devflow: ${DEVFLOW_SESSION_SCOPE_UNAVAILABLE}: session ${sessionId} has no workspace to derive `
 ...
 */
export function deriveProjectNameFromWorkspace(workspace) {
 ...
 */
export function deriveNewProjectIdentity(workspace) {
 ...
 */
export function deriveSessionStoreRoot(workspace, defaultRoot, sessionId) {
    if (workspace === undefined) {
        throw new DevFlowSessionScopeError(sessionId, 'the session header carries no workspace (cwd), so no project can be derived');
    }
    if (workspace.trim() === '') {
...
export class DevFlowSessionStores {
    ...
    scopeOf(root, workspacePath, sessionId, agent) {
        ...
        if (store === undefined) {
            ...
            store = new DevFlowStore(this.fs, root, this.onChange === undefined
                ? undefined
                : (relativePath, sequence, record) => this.onChange?.(sessionKey, relativePath, sequence, record), this.sandboxPolicyOf === undefined || boundAgent === undefined
                ? undefined
