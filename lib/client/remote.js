import { DEVFLOW_CLIENT_AUDIT_PAGE_BYTES, DEVFLOW_CLIENT_AUDIT_PAGE_LIMIT, DEVFLOW_CLIENT_AUDIT_PAGE_VERSION, DEVFLOW_CLIENT_CHANGE_KINDS, DEVFLOW_CLIENT_EVENT_CHANGE_LIMIT, DEVFLOW_CLIENT_EVENT_PATH_LIMIT, } from "../contract.js";
const agentParameter = {
    name: 'agent', wire: 'agentId', source: 'lookup', lookup: 'agent',
    codec: { mode: 'strict', typeSymbol: 'session-id', schema: { parse: (value) => requireString(value, 'session id') } },
};
/** Client-side Remote contribution for DevFlow's narrow public bridge. */
export const DEVFLOW_REMOTE = {
 ...
 */
export function parseFollowRequest(value) {
    if (!isRecord(value))
...
export function parseDevFlowEvent(value) {
    if (!isRecord(value) || typeof value.kind !== 'string')
        throw new Error('invalid DevFlow event');
    if (value.kind === 'snapshot') {
        const parsed = parseDevFlowResponse({ kind: 'snapshot', snapshot: value.snapshot });
        if (parsed.kind !== 'snapshot')
            throw new Error('invalid DevFlow event snapshot');
        return { kind: 'snapshot', snapshot: parsed.snapshot };
    }
    if (value.kind !== 'changed')
        throw new Error('invalid DevFlow event kind');
    const revision = value.revision;
    if (typeof revision !== 'number' || !Number.isSafeInteger(revision) || revision < 0)
        throw new Error('invalid DevFlow event revision');
    const sequence = value.sequence;
    ...
    if (typeof value.at !== 'string' || !validTime(value.at))
        throw new Error('invalid DevFlow event time');
    if (!Array.isArray(value.changed) || value.changed.length > DEVFLOW_CLIENT_EVENT_PATH_LIMIT
        || !value.changed.every(item => isBoundedString(item, 240)))
        throw new Error('invalid DevFlow event paths');
    return {
        kind: 'changed',
        revision,
...
export function parseDevFlowResponse(value) {
    if (!isRecord(value) || typeof value.kind !== 'string')
    ...
    if (value.kind === 'error') {
        ...
        if (value.error.code !== 'state-unavailable'
            || value.error.message !== 'DevFlow state is unavailable. Refresh to try again.')
            throw new Error('invalid DevFlow bridge error');
    ...
    if (snapshot.version !== 1 || typeof snapshot.generatedAt !== 'string' || !isRecord(snapshot.session)
        ...
        || !optionalArray(snapshot.blocked, isBlockedRow))
    ...
    return {
        kind: 'snapshot', snapshot: {
            ...
            project: snapshot.project, agents: snapshot.agents,
            tasks: snapshot.tasks, phases: snapshot.phases,
            assignments: snapshot.assignments, executions: snapshot.executions,
            ...(snapshot.results === undefined ? {} : { results: snapshot.results.map(normalizeResultSummary) }),
            ...(snapshot.reports === undefined ? {} : { reports: snapshot.reports.map(normalizeReportSummary) }),
            ...(snapshot.attempts === undefined ? {} : { attempts: snapshot.attempts.map(normalizeAttemptSummary) }),
            ...(snapshot.failures === undefined ? {} : { failures: snapshot.failures.map(normalizeFailureSummary) }),
            // This rebuild keeps only the fields it names, so the optional 受阻 rows
            // the host sends would otherwise never reach the canvas.
            ...(snapshot.blocked === undefined ? {} : { blocked: snapshot.blocked.map(normalizeBlockedRow) }),
            decisions: snapshot.decisions, decisionRequests: snapshot.decisionRequests,
        },
    };
}
/** Strictly parse a browser-supplied safe page query; unknown fields are dropped. */
export function parseAuditQuery(value) {
    if (!isRecord(value))
        throw new Error('invalid DevFlow audit query');
    ...
    if (value.range !== undefined) {
        if (!isRecord(value.range))
            throw new Error('invalid DevFlow audit range');
...
export function parseDevFlowAuditResponse(value) {
    if (!isRecord(value) || typeof value.kind !== 'string')
        throw new Error('invalid DevFlow audit response');
    if (value.kind === 'error')
        return { kind: 'error', error: parseAuditError(value.error) };
    if (value.kind !== 'page' || !isRecord(value.page))
        throw new Error('invalid DevFlow audit response');
    ...
    if (page.version !== DEVFLOW_CLIENT_AUDIT_PAGE_VERSION || page.source !== 'devflow-journal'
        ...
        || !isNonNegativeInteger(page.omittedUnsafeCount) || typeof page.truncated !== 'boolean' || !page.items.every(isAuditItem))
        throw new Error('invalid DevFlow audit page');
    const normalized = {
        version: DEVFLOW_CLIENT_AUDIT_PAGE_VERSION, source: 'devflow-journal', projectId: page.projectId,
        range: { from: page.range.from, to: page.range.to }, items: page.items.map(normalizeAuditItem),
        nextCursor: page.nextCursor, capturedHeadSequence: page.capturedHeadSequence,
        omittedUnsafeCount: page.omittedUnsafeCount, truncated: page.truncated,
    };
    if (new TextEncoder().encode(JSON.stringify(normalized)).byteLength > DEVFLOW_CLIENT_AUDIT_PAGE_BYTES)
        throw new Error('invalid DevFlow audit page');
...
function parseAuditError(value) {
    ...
    if (value.code === 'audit-unavailable' && value.message === 'DevFlow audit is unavailable. Refresh to try again.')
        return { code: value.code, message: value.message };
    if (value.code === 'cursor-invalid' && value.message === 'DevFlow audit history changed. Refresh to try again.')
        return { code: value.code, message: value.message };
    throw new Error('invalid DevFlow audit error');
}
...
function normalizeSafeText(value) {
    return { text: value.text, truncated: value.truncated, redacted: value.redacted };
}
...
function normalizeSessionState(value) {
    ...
    if (presetId !== null && !isBoundedString(presetId, 128))
        throw new Error('invalid DevFlow session presetId');
    ...
    if (verifiedAt !== null && !validTime(verifiedAt))
        throw new Error('invalid DevFlow session verifiedAt');
    ...
    if (workspacePath !== null && !isBoundedString(workspacePath, 512))
        throw new Error('invalid DevFlow session workspacePath');
    const storeRoot = value.storeRoot === undefined || value.storeRoot === null ? null : value.storeRoot;
    if (storeRoot !== null && !isBoundedString(storeRoot, 512))
        throw new Error('invalid DevFlow session storeRoot');
    return {
        id: value.id,
...
function normalizeResultSummary(value) { const item = value; return { id: item.id, taskId: item.taskId, source: 'result', at: item.at, summary: normalizeSafeText(item.summary) }; }
function normalizeReportSummary(value) { const item = value; return { id: item.id, executionId: item.executionId, ...(item.taskId === undefined ? {} : { taskId: item.taskId }), agentId: item.agentId, source: 'report', status: item.status, at: item.at, summary: normalizeSafeText(item.summary) }; }
function normalizeAttemptSummary(value) { const item = value; return { id: item.id, executionId: item.executionId, source: 'attempt', status: item.status, isRetry: item.isRetry, at: item.at, completedAt: item.completedAt }; }
function normalizeFailureSummary(value) { const item = value; return { id: item.id, executionId: item.executionId, ...(item.taskId === undefined ? {} : { taskId: item.taskId }), agentId: item.agentId, source: item.source, status: item.status, at: item.at, summary: item.summary === null ? null : normalizeSafeText(item.summary) }; }
function normalizeAuditItem(value) {
    const item = value;
    const entity = item.entity;
    const related = item.related;
    return {
        id: item.id, sequence: item.sequence, category: item.category, action: item.action, at: item.at,
        entity: { type: entity.type, id: entity.id, display: entity.display === null ? null : normalizeSafeText(entity.display) },
        related: pickRelated(related), status: item.status, summary: item.summary === null ? null : normalizeSafeText(item.summary), incomplete: item.incomplete,
    };
}
function pickRelated(value) { const allowed = ['projectId', 'taskId', 'phaseId', 'agentId', 'assignmentId', 'executionId', 'attemptId', 'reportId', 'decisionId', 'requestId']; return Object.fromEntries(allowed.flatMap(key => typeof value[key] === 'string' ? [[key, value[key]]] : [])); }
function optionalSummaries(snapshot) { return optionalArray(snapshot.results, isResultSummary) && optionalArray(snapshot.reports, isReportSummary) && optionalArray(snapshot.attempts, isAttemptSummary) && optionalArray(snapshot.failures, isFailureSummary); }
function optionalArray(value, predicate) { return value === undefined || (Array.isArray(value) && value.length <= 20 && value.every(predicate)); }
function isSafeText(value) { return isRecord(value) && typeof value.text === 'string' && value.text.length <= 500 && typeof value.truncated === 'boolean' && typeof value.redacted === 'boolean'; }
function isResultSummary(value) { return isRecord(value) && typeof value.id === 'string' && typeof value.taskId === 'string' && value.source === 'result' && typeof value.at === 'string' && isSafeText(value.summary); }
function isReportSummary(value) { return isRecord(value) && typeof value.id === 'string' && typeof value.executionId === 'string' && (value.taskId === undefined || typeof value.taskId === 'string') && typeof value.agentId === 'string' && value.source === 'report' && (value.status === 'success' || value.status === 'failed' || value.status === 'blocked') && typeof value.at === 'string' && isSafeText(value.summary); }
function isAttemptSummary(value) { return isRecord(value) && typeof value.id === 'string' && typeof value.executionId === 'string' && value.source === 'attempt' && (value.status === 'created' || value.status === 'running' || value.status === 'completed' || value.status === 'failed') && typeof value.isRetry === 'boolean' && typeof value.at === 'string' && (value.completedAt === null || typeof value.completedAt === 'string'); }
function isFailureSummary(value) { return isRecord(value) && typeof value.id === 'string' && typeof value.executionId === 'string' && (value.taskId === undefined || typeof value.taskId === 'string') && typeof value.agentId === 'string' && (value.source === 'execution' || value.source === 'report') && (value.status === 'failed' || value.status === 'blocked') && typeof value.at === 'string' && (value.summary === null || isSafeText(value.summary)); }
function isBlockedRow(value) { return isRecord(value) && typeof value.id === 'string' && typeof value.taskId === 'string' && typeof value.agentId === 'string' && typeof value.agentName === 'string' && GAP_KINDS.includes(value.gapKind) && typeof value.missing === 'string' && typeof value.suggestedOwner === 'string' && isSafeText(value.reason) && typeof value.headline === 'string' && validTime(value.at); }
function normalizeBlockedRow(value) {
    const item = value;
    return {
        id: item.id, taskId: item.taskId, agentId: item.agentId, agentName: item.agentName,
        gapKind: item.gapKind, missing: item.missing, suggestedOwner: item.suggestedOwner,
        reason: normalizeSafeText(item.reason), headline: item.headline, at: item.at,
    };
}
...
function isAuditItem(value) { if (!isRecord(value) || !isBoundedId(value.id) || !isSequence(value.sequence) || !CATEGORIES.includes(value.category) || !ACTIONS.includes(value.action) || !validTime(value.at) || !isRecord(value.entity) || !ENTITY_TYPES.includes(value.entity.type) || (value.entity.id !== null && !isBoundedId(value.entity.id)) || (value.entity.display !== null && !isSafeText(value.entity.display)) || !isRecord(value.related) || (value.status !== null && (!isBoundedString(value.status, 64))) || (value.summary !== null && !isSafeText(value.summary)) || typeof value.incomplete !== 'boolean')
    return false; return Object.values(value.related).every(isBoundedId); }
function requireTime(value) { if (!validTime(value))
    throw new Error('invalid DevFlow audit time'); return value; }
function requireString(value, label) { if (typeof value !== 'string' || value === '')
    throw new Error(`invalid ${label}`); return value; }
function requireBoundedString(value, label, max) { if (!isBoundedString(value, max))
    throw new Error(`invalid ${label}`); return value; }
function isBoundedString(value, max) { return typeof value === 'string' && value.length > 0 && value.length <= max && !/[\x00-\x1F]/.test(value); }
function isBoundedId(value) { return isBoundedString(value, 128); }
function validTime(value) { return typeof value === 'string' && value.length <= 64 && Number.isFinite(Date.parse(value)); }
function isSequence(value) { return typeof value === 'number' && Number.isSafeInteger(value) && value >= 0; }
function isNonNegativeInteger(value) { return isSequence(value); }
