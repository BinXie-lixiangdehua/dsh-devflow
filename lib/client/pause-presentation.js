 ...
 */
const IN_FLIGHT_EDGE = new Set(['executing', 'rework', 'queued']);
/**
 ...
 */
const IN_FLIGHT_NODE = new Set(['active', 'rework', 'planned', 'pending']);
/**
 ...
 */
export function applyPausePresentation(model, paused) {
    if (!paused)
        return model;
    const edges = model.edges.map(edge => {
        if (!IN_FLIGHT_EDGE.has(edge.state))
            return edge;
        return {
            ...edge,
            state: 'paused',
    ...
    const nodes = model.nodes.map(node => (isPausableNode(node) ? { ...node, state: 'paused', stateLabel: PAUSED_LABEL } : node));
...
function isPausableNode(node) {
    if (node.kind === 'requirement')
        return false;
    if (node.kind === 'commander' && node.state === 'planned')
        return false;
