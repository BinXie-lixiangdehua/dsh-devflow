import { DEVFLOW_REMOTE } from "./remote.js";
import { DEVFLOW_ID, DEVFLOW_KIND, devflowDefinition } from "./sidebar-definition.js";
import { DevFlowCanvas } from "./DevFlowCanvas.js";
import { DevFlowCanvasTitle } from "./DevFlowCanvasTitle.js";
import { DevFlowOverview, OVERVIEW_REOPEN_EVENT } from "./DevFlowOverview.js";
import { OVERVIEW_CELL_ID } from "./overview.js";
import { DevFlowSnapshotController, DevFlowLiveController } from "./store.js";
import { installDevFlowStyles } from "./styles.js";
const NS = 'devflow';
...
export function apply(ctx) {
    ...
    const scopedRemoteFor = (sessionId) => {
        ...
        if (agentCtx === undefined || !remoteReady)
            return undefined;
    ...
    const refreshControllers = () => {
        for (const [sessionId, controller] of controllers)
            controller.setRemote(remoteFor(sessionId));
        for (const [sessionId, live] of liveControllers)
            live.setRemote(remoteFor(sessionId));
    };
    ctx.effect(() => {
        ...
        const disposeCanvas = ctx.slots.inject('sidebar.right.pane.tab', () => ctx.slots.register({
            ...
            inject: (sessionId) => {
                ...
                return {
                    ...
                    setInspectorTab: (tab) => { controller.setInspectorTab(tab); },
                    audit: {
        ...
        const syncType = () => {
            ...
            if (wanted && disposeType === null)
                disposeType = ctx.sidebarRightTabs.register(devflowDefinition(ctx.locale.bind(NS)));
        ...
        const openPanel = () => {
            try {
                if (document.documentElement.dataset.devflowPresetWanted !== 'true')
                    return;
        ...
        return () => {
            ...
            disposeTitle();
            disposeCanvas();
            for (const live of liveControllers.values())
                live.dispose();
