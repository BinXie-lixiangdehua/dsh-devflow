...
const STILL = { flowing: false, multiplier: 0, speed: 0, durationSeconds: 0, opacity: 1 };
/**
 ...
 */
const PAUSED = { flowing: false, multiplier: 0, speed: 0, durationSeconds: 0, opacity: 1, paused: true };
/**
 ...
 */
export function motionProfile(state, paused = false) {
    if (paused && (state === 'executing' || state === 'rework' || state === 'queued' || state === 'paused'))
        return PAUSED;
    if (state === 'executing')
        return profile(1);
    if (state === 'rework')
        return profile(1.4);
    if (state === 'queued')
        return { ...profile(0.5), opacity: FLOW_FLOW_OPACITY_WEAK };
...
export function motionRate(profile) {
 ...
 */
export function freshlyDoneEdges(previous, edges) {
    const fresh = [];
    const seen = new Set();
    for (const edge of edges) {
        seen.add(edge.id);
        const before = previous.get(edge.id);
        if (edge.state === 'done' && before !== undefined && before !== 'done')
            fresh.push(edge.id);
 ...
 */
export function changedEdges(previous, edges) {
    const changed = [];
    const seen = new Set();
    for (const edge of edges) {
        seen.add(edge.id);
        const before = previous.get(edge.id);
        if (before !== undefined && before !== edge.state)
 ...
 */
export const FLOW_ENTER_HISTORY_START = new Set();
/**
 ...
 */
export function newlyEnteredNodes(previousIds, nodes) {
