...
import { shortId, taskWorkStateLabel, } from "./workspace.js";
/**
...
export const FLOW_SEMANTIC_LABELS = {
    requirement: '需求下达',
    dispatch: '派发',
    delivery: '交付 · 汇报',
    rework: '返工',
 ...
 */
export function flowAgentLabel(agentId, role, fallback, namedByItself = false) {
...
export function flowNodeStateLabel(state) { return NODE_STATE_LABELS[state]; }
export function flowEdgeStateLabel(state) { return EDGE_STATE_LABELS[state]; }
export function flowSemanticLabel(semantic) { return FLOW_SEMANTIC_LABELS[semantic]; }
/**
 ...
 *   corridor heights.
 */
export function createFlowModel(model, now = Date.now(), nodeHeights = {}, scope = 'current') {
    ...
    const context = { now, newestCompletion: newestCompletionAt(model) };
...
function tasksOfPhase(model, phaseId) {
    if (phaseId === '' || phaseId === 'phase:unrecorded')
        return [];
    return model.tasks
...
function orderEmployees(agents, routed) {
    ...
    for (const edge of routed) {
        ...
        if (current === undefined || edge.order < current)
            score.set(edge.targetAgentId, edge.order);
    }
 ...
 */
export function countCrossings(shapes) {
    const TOP = -1;
    ...
    for (let a = 0; a < segments.length; a += 1) {
        for (let b = 0; b < segments.length; b += 1) {
            ...
            for (const vertical of second.verticals) {
                ...
                if (vertical.x1 <= low || vertical.x1 >= high)
                    continue;
...
function semanticOf(edge, target) {
    if (target?.agent.kind === 'temporary')
        return 'subagent';
...
function phaseOrder(edges) {
    ...
    for (const edge of edges) {
        ...
        if (current === undefined || value < current)
            firstSeen.set(edge.phaseId, value);
    }
...
function pickCurrentEdges(edges) {
    ...
    for (const edge of edges) {
        ...
        if (list === undefined)
            byAgent.set(edge.targetAgentId, [edge]);
        else
            list.push(edge);
    }
...
function newestCompletionAt(model) {
    ...
    for (const execution of model.snapshot.executions) {
        ...
        if (!Number.isNaN(parsed) && parsed > newest)
            newest = parsed;
    }
    return newest;
}
...
function edgeState(taskState, assignment, execution, retryCount, dispatchAt, context) {
    ...
    if (closure !== null)
        return { state: 'closed', reason: closure.reason };
    ...
    if (execution !== null && execution.status === 'failed')
        return { state: 'rework', reason: null };
    if (retryCount > 0 && (taskState === 'failed' || taskState === 'working' || taskState === 'blocked'))
        return { state: 'rework', reason: null };
    if (taskState === 'failed')
        return { state: 'rework', reason: null };
    ...
    if (Number.isNaN(queuedAt) || queuedAt === 0)
        return { state: 'queued', reason: null };
...
function edgeAction(state, taskState, assignment) {
    if (state === 'rework')
        return '派发返工';
    if (state === 'done')
        return assignment?.role === 'reviewer' ? '复核通过' : '交付结果';
...
function nodeState(item, current, edges) {
    ...
    if (edges.some(edge => edge.targetAgentId === item.agent.id && edge.state === 'executing'))
        return 'active';
    if (item.workState === 'blocked')
        return 'blocked';
    if (item.workState === 'working')
        return 'active';
    if (current !== null)
        return current.state === 'rework' ? 'rework' : current.state === 'lost' ? 'lost' : 'planned';
    if (item.workState === 'done')
        return 'done';
    if (item.workState === 'archived')
        return 'done';
    return 'idle';
}
 ...
 */
export function routeEdge(from, to, fromPort, toPort, laneY, boxes) {
    const fromBox = boxes.get(from);
    const toBox = boxes.get(to);
    if (fromBox === undefined || toBox === undefined) {
...
export function layoutEdges(edges, boxes) {
...
export function orthogonalPath(points, radius = 5) {
    if (points.length === 0)
        return '';
...
function boundText(text, limit) {
    ...
    if (value.length === 0)
        return { text: '未记录', truncated: false };
