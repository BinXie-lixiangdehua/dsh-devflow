...
export const PROJECT_SELECTION: WorkspaceSelection = { kind: 'project' }

/** Convert a Canvas selection into an exact, Host-enforced audit predicate. */
export function auditFilterForSelection(selection: WorkspaceSelection): DevFlowClientAuditFilter {
  if (selection.kind === 'project' || selection.kind === 'commander') return { kind: 'project' }
  if (selection.kind === 'phase') return { kind: 'phase', id: selection.id }
  if (selection.kind === 'task') return { kind: 'task', id: selection.id }
  if (selection.kind === 'agent') return { kind: 'agent', id: selection.id }
  if (selection.kind === 'execution') return { kind: 'execution', id: selection.id }
  return { kind: 'decision', id: selection.id }
}

/** Build the read-only Canvas view model from the current V1 snapshot only. */
export function createWorkspaceModel(snapshot: DevFlowClientSnapshot): WorkspaceModel {
  ...
  const pendingDecisionsByTask = groupBy(
    snapshot.decisionRequests.filter(hasPendingTaskId),
...
export function selectionExists(model: WorkspaceModel, selection: WorkspaceSelection): boolean {
...
export function flowAgentName(agentId: string, displayName: string): string {
  return AGENT_DISPLAY_NAMES[agentId] ?? displayName
}

/** Render-safe title for the selected object. */
export function selectionLabel(model: WorkspaceModel, selection: WorkspaceSelection): string {  if (selection.kind === 'project') return model.snapshot.project?.name ?? 'Shared project'
  if (selection.kind === 'commander') return 'Commander'
  if (selection.kind === 'phase') return model.phases.find(phase => phase.id === selection.id)?.name ?? selection.id
  if (selection.kind === 'task') return model.taskById.get(selection.id)?.task.title ?? selection.id
  if (selection.kind === 'agent') return model.agentById.get(selection.id)?.agent.displayName ?? selection.id
  if (selection.kind === 'execution') return `Execution ${shortId(selection.id)}`
...
export function selectionParent(model: WorkspaceModel, selection: WorkspaceSelection): WorkspaceSelection | undefined {
  if (selection.kind === 'project') return undefined
  if (selection.kind === 'commander') return PROJECT_SELECTION
  if (selection.kind === 'phase' || selection.kind === 'agent') return PROJECT_SELECTION
  ...
  if (selection.kind === 'execution') {
    ...
    if (execution?.taskId !== undefined) return { kind: 'task', id: execution.taskId }
    return execution === undefined ? PROJECT_SELECTION : { kind: 'agent', id: execution.agentId }
  }
  const request = model.decisionRequestsById.get(selection.id)
  if (request?.taskId !== null && request?.taskId !== undefined) return { kind: 'task', id: request.taskId }
  if (model.decisionsById.has(selection.id)) return PROJECT_SELECTION
  return PROJECT_SELECTION
}

/** All visible phase IDs relevant to a selection. Empty means show the full project view. */
export function focusedPhaseIds(model: WorkspaceModel, selection: WorkspaceSelection): ReadonlySet<string> {
  if (selection.kind === 'phase') return new Set([selection.id])
  if (selection.kind === 'task') return new Set(model.taskById.get(selection.id)?.phaseIds ?? [])
...
export function isSelectionRelated(model: WorkspaceModel, selection: WorkspaceSelection, taskId: string): boolean {
  if (selection.kind === 'project' || selection.kind === 'commander') return true
  if (selection.kind === 'task') return selection.id === taskId
  if (selection.kind === 'phase') return model.taskById.get(taskId)?.phaseIds.includes(selection.id) ?? false
  if (selection.kind === 'agent') return model.agentById.get(selection.id)?.taskIds.includes(taskId) ?? false
  if (selection.kind === 'execution') return model.executionById.get(selection.id)?.taskId === taskId
...
export function lifecycleLabel(status: DevFlowClientAgent['status']): string {
  return ({ active: 'Active', created: 'Created', running: 'Running', terminated: 'Terminated' })[status]
}

export function taskWorkStateLabel(state: TaskWorkState): string {
  return ({
 ...
 */
function taskWorkState(
  task: DevFlowClientTask,
  executions: readonly DevFlowClientExecution[],
  pendingDecisions: readonly DevFlowClientDecisionRequest[],
): TaskWorkState {
 ...
 */
function newestExecution(executions: readonly DevFlowClientExecution[]): DevFlowClientExecution | null {
...
function agentWorkState(
  agent: DevFlowClientAgent,
  executions: readonly DevFlowClientExecution[],
  pendingDecisions: readonly DevFlowClientDecisionRequest[],
...
function createBlockers(snapshot: DevFlowClientSnapshot, tasks: readonly WorkspaceTask[]): WorkspaceBlocker[] {
...
function createRecentActivity(snapshot: DevFlowClientSnapshot): readonly WorkspaceActivity[] {
  const activity: WorkspaceActivity[] = [
    ...snapshot.tasks.map(task => ({
      id: `task-${task.id}`, at: task.updatedAt, title: 'Task updated', detail: task.title,
      selection: { kind: 'task' as const, id: task.id },
    })),
    ...snapshot.executions.flatMap(execution => [
      ...(execution.startedAt === null ? [] : [{
        id: `execution-start-${execution.id}`, at: execution.startedAt, title: 'Execution started',
        detail: `Execution ${shortId(execution.id)}`, selection: { kind: 'execution' as const, id: execution.id },
      }]),
    ...
    ...snapshot.decisions.map(decision => ({
      ...
      selection: { kind: 'decision' as const, id: decision.id },
    })),
  ]
  return activity
...
function hasPendingTaskId(request: DevFlowClientDecisionRequest): request is DevFlowClientDecisionRequest & { readonly taskId: string } {
  return request.status === 'pending' && request.taskId !== null
}

function hasTaskId<T extends { readonly taskId?: string }>(item: T): item is T & { readonly taskId: string } {
