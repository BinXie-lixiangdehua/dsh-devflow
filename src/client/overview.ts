 ...
 */
export function overviewClosedKey(sessionId: string | null | undefined): string {
 ...
 */
export function overviewExpandedKey(sessionId: string | null | undefined): string {
  return sessionId === null || sessionId === undefined || sessionId === ''
 ...
 */
export function legacyFlagCleanup(storage: { removeItem(key: string): void } | null): void {
...
export interface OverviewSessionInput {
  ...
  readonly byId: Readonly<Record<string, { readonly projectionValues?: Readonly<Partial<Record<string, unknown>>> } | undefined>>
}
 ...
 */
export function overviewGate(sessions: OverviewSessionInput | undefined | null): OverviewGate {
  ...
  if (typeof raw !== 'string' || raw === '') return { allowed: false, sessionId, preset: null, reason: 'preset-unresolved' }
  if (raw !== DEVFLOW_PRESET) return { allowed: false, sessionId, preset: raw, reason: 'other-preset' }
...
export function connectionLabel(connection: DevFlowConnectionState | null): string {
...
export function hiddenDispatchCount(flow: FlowModel): number {
...
export function buildOverview(
  ...
  now?: number,
  connection: DevFlowConnectionState | null = null,
): OverviewView {
 ...
 */
function countEdges(edges: readonly FlowEdge[], model: WorkspaceModel): OverviewCounts {
  let active = 0
  let review = 0
  let done = 0
  let lost = 0
  ...
  for (const edge of edges) {
    if (edge.state === 'executing' || edge.state === 'queued') { active += 1; continue }
    if (edge.state === 'rework') { active += 1; continue }
    if (edge.state === 'paused') { paused += 1; continue }
    if (edge.state === 'lost') { lost += 1; continue }
    // 已收尾 is its OWN bucket: it must not inflate 未收尾 · 已失联, which is the whole
    // point of the round's change to the counts.
    if (edge.state === 'closed') { closed += 1; continue }
    if (edge.state === 'done') {
...
function summaryLine(lost: number, active: number, review: number, done: number): string {
  const parts: string[] = []
 ...
 */
function postureOf(counts: OverviewCounts): 'active' | 'review' | 'done' {
  if (counts.active > 0 || counts.lost > 0 || counts.paused > 0) return 'active'
  if (counts.review > 0) return 'review'
  return 'done'
}

function postureLabel(posture: 'active' | 'review' | 'done'): string {
  return posture === 'active' ? '进行中' : posture === 'review' ? '待验收' : '全部完成'
}

/** One employee's row: the node the canvas draws, plus the dispatch it holds. */
function memberRow(agent: DevFlowClientAgent, flow: FlowModel): OverviewMember {
