 ...
 */
export const FLOW_DASH_HEAD = 32
/** Dash offset of one period used by the animation and asserted as the period. */
export const FLOW_DASH_TAIL = FLOW_DASH_PERIOD - FLOW_DASH_HEAD
/**
 ...
 */
export const FLOW_GLOW_LIGHT = '#3fbfe0'
export const FLOW_GLOW_GOLD = '#7fd4ea'
/** Rework head: the same lift applied to the red family, per skin. */
export const FLOW_GLOW_REWORK_LIGHT = '#e8655f'
export const FLOW_GLOW_REWORK_GOLD = '#ff8a86'
/** Stroke width of the moving highlight, in SVG user units (was 2.6). */
export const FLOW_FLOW_WIDTH = 3.4
/** Queue's highlight stays deliberately weaker than 执行中's (the 3C layering rule). */
export const FLOW_FLOW_WIDTH_WEAK = 3.0
export const FLOW_FLOW_OPACITY_WEAK = 0.55
...
const STILL: FlowMotionProfile = { flowing: false, multiplier: 0, speed: 0, durationSeconds: 0, opacity: 1 }
/**
 ...
 */
const PAUSED: FlowMotionProfile = { flowing: false, multiplier: 0, speed: 0, durationSeconds: 0, opacity: 1, paused: true }
 ...
 */
export function motionProfile(state: FlowEdgeState, paused = false): FlowMotionProfile {
  ...
  if (state === 'queued') return { ...profile(0.5), opacity: FLOW_FLOW_OPACITY_WEAK }
  // 已完成 gets its one-shot sweep (handled by {@link freshlyDoneEdges}), 未收尾 stays still.
  return STILL
}

function profile(multiplier: number): FlowMotionProfile {
  const speed = FLOW_LINE_SPEED_BASE * multiplier
  return {
    flowing: true,
    multiplier,
    speed: Math.round(speed * 10) / 10,
    durationSeconds: Math.round((FLOW_DASH_PERIOD / speed) * 1000) / 1000,
    opacity: 1,
  }
}
...
export function motionRate(profile: FlowMotionProfile): FlowMotionRate {
 ...
 */
export function freshlyDoneEdges(
  previous: Map<string, FlowEdgeState>,
  edges: readonly { readonly id: string; readonly state: FlowEdgeState }[],
): readonly string[] {
  const fresh: string[] = []
  ...
  for (const edge of edges) {
    ...
    if (edge.state === 'done' && before !== undefined && before !== 'done') fresh.push(edge.id)
 ...
 */
export const FLOW_ENTER_MS = 420
...
export function changedEdges(
  ...
  for (const edge of edges) {
    seen.add(edge.id)
    const before = previous.get(edge.id)
    if (before !== undefined && before !== edge.state) changed.push(edge.id)
    previous.set(edge.id, edge.state)
  }
  for (const id of [...previous.keys()]) {
    if (!seen.has(id)) previous.delete(id)
 ...
 */
export const FLOW_ENTER_HISTORY_START: ReadonlySet<string> = new Set<string>()
 ...
 */
export function newlyEnteredNodes(
 ...
 */
export function nodeIdSet(nodes: readonly { readonly id: string }[]): ReadonlySet<string> {
