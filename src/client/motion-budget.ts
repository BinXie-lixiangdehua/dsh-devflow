...
/** The posture the canvas root publishes. */
export type DevFlowGlassBudget = 'auto' | 'degrade'
export type DevFlowMotionState = 'idle' | 'active' | 'degraded'

/** A manual override, so a reviewer can force either path in a real browser. */
export type DevFlowGlassForce = 'auto' | 'degrade' | 'glass'
 ...
 */
export function readGlassOverride(search: string): DevFlowGlassForce {
...
/** Everything one sample of the environment reports. */
export interface MotionSample {
  /** True when at least one flow layer is animating right now. */
  readonly animating: boolean
  /** Mean frame time over the sample window, in ms. */
  readonly meanFrameMs: number
}

/** The decision one sample produces. */
export interface MotionBudgetDecision {
  readonly budget: DevFlowGlassBudget
  readonly motion: DevFlowMotionState
  /** True while the adaptive long-frame rule is holding degradation. */
...
export class MotionBudget {
   ...
   */
  sample(sample: MotionSample): MotionBudgetDecision {
    if (Number.isFinite(sample.meanFrameMs)) {
      if (sample.meanFrameMs > LONG_FRAME_MS) { this.long++; this.fast = 0 } else if (sample.meanFrameMs < RECOVER_MS) { this.fast++; this.long = 0 } else { this.long = 0; this.fast = 0 }
      ...
      if (this.long >= LONG_FRAME_STREAK) this.slow = true
      else if (this.fast >= LONG_FRAME_STREAK) this.slow = false
    }
    const budget: DevFlowGlassBudget = sample.animating || this.slow ? 'degrade' : 'auto'
    const motion: DevFlowMotionState = this.slow ? 'degraded' : sample.animating ? 'active' : 'idle'
    return { budget, motion, slow: this.slow }
  }
}

/** Mean of a frame-gap list; `null` when nothing was sampled. */
export function meanFrameMs(gaps: readonly number[]): number | null {
