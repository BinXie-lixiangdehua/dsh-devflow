...
function readFlag(storage: Pick<Storage, 'getItem'> | null, key: string): boolean {
...
function writeFlag(storage: Pick<Storage, 'setItem' | 'removeItem'> | null, key: string, folded: boolean): void {
 ...
 */
export function activationFoldKey(failure: RefusalIdentity): string {
 ...
 */
export function activationBannerMode(
  failure: RefusalIdentity | null,
  activation: DevFlowClientActivation,
  storage: Pick<Storage, 'getItem'> | null,
): { readonly collapsible: boolean; readonly folded: boolean } {
  if (failure === null) return { collapsible: false, folded: false }
  const collapsible = activation === 'bound'
  if (!collapsible) return { collapsible, folded: false }
 ...
 */
export function storeActivationFold(
  storage: Pick<Storage, 'setItem' | 'removeItem'> | null,
  failure: RefusalIdentity,
  folded: boolean,
): void {
 ...
 */
export function blockedExpandKey(rows: BlockedIdentity): string {
 ...
 */
export function blockedBannerFolded(rows: BlockedIdentity, storage: Pick<Storage, 'getItem'> | null): boolean {
 ...
 */
export function storeBlockedFold(
  storage: Pick<Storage, 'setItem' | 'removeItem'> | null,
  rows: BlockedIdentity,
  folded: boolean,
): void {
