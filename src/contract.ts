...
export const DEVFLOW_CLIENT_SNAPSHOT_VERSION = 1 as const
 ...
 */
export const DEVFLOW_CONCURRENCY_LIMIT = 5 as const

export type DevFlowClientTaskStatus =
  | 'created' | 'planned' | 'executing' | 'reviewing' | 'completed' | 'failed' | 'cancelled'
...
export interface DevFlowClientStateError {
  ...
  readonly message: 'DevFlow state is unavailable. Refresh to try again.'
    | 'DevFlow cannot isolate this session: it has no project workspace. Shared state is not shown.'
}

export type DevFlowClientSnapshotResponse =
  | { readonly kind: 'snapshot'; readonly snapshot: DevFlowClientSnapshot }
  | { readonly kind: 'error'; readonly error: DevFlowClientStateError }

/** P1A is independently versioned so V1 snapshot/refresh stays stable. */
export const DEVFLOW_CLIENT_AUDIT_PAGE_VERSION = 1 as const
export const DEVFLOW_CLIENT_AUDIT_PAGE_LIMIT = 20 as const
export const DEVFLOW_CLIENT_AUDIT_TEXT_LIMIT = 500 as const
export const DEVFLOW_CLIENT_AUDIT_PAGE_BYTES = 49_152 as const
...
export type DevFlowClientAuditCategory =
  | 'project' | 'task' | 'phase' | 'agent' | 'assignment' | 'execution'
  | 'attempt' | 'report' | 'decision' | 'control' | 'scope' | 'bridge-review'
  | 'runtime' | 'commander-action'

export type DevFlowClientAuditAction =
  | 'updated' | 'created' | 'removed' | 'transitioned' | 'assigned' | 'unassigned'
  | 'started' | 'completed' | 'failed' | 'blocked' | 'requested' | 'answered'
  | 'paused' | 'resumed' | 'imported' | 'exported' | 'boundary-hit' | 'executed'
  /** A stale record was closed: terminal, never continues (the 收尾终态 statement). */
  | 'closed'

export type DevFlowClientAuditEntityType =
  | 'project' | 'task' | 'phase' | 'agent' | 'assignment' | 'execution' | 'attempt'
  | 'report' | 'decision-request' | 'decision' | 'batch' | 'runtime-session'
...
export type DevFlowClientAuditPageResponse =
  | { readonly kind: 'page'; readonly page: DevFlowClientAuditPage }
  | { readonly kind: 'error'; readonly error: DevFlowClientAuditError }
...
export const DEVFLOW_CLIENT_CHANGE_KINDS: readonly string[] = [
