...
import { registerDevFlowTools } from './tools.ts'
...
export class DevflowController extends Service {
  ...
  private durableState: DevFlowStoreState = initialDevFlowState()
  ...
  constructor(ctx: Context, config: DevFlowConfig = {}) {
    ...
    this.sessionStores = new DevFlowSessionStores(
      ctx.fs,
      devflowDir,
      (sessionKey, relativePath, sequence, record) => {
    ...
    if (commander === undefined) throw new Error('devflow: default commander configuration is missing')
    ...
    this.presetActivation = new DevFlowPresetActivation(
      this.sessionStores,
      this.commanderMode,
      {
