...
import type {
  AgentConfigPatch, AgentInstance, AgentReport, AgentReportUpdate, AssignmentStatus, BlockedReport, DevFlowCloseReason,
  CommanderAction, CommanderActionStatus, CommanderCheckpoint, CommanderCheckpointUpdate,
  CommanderDecision, CommanderDecisionUpdate, CommanderActionExecutionRecord, CommanderExecutionContext,
  CommanderMemory,
  CommanderMemoryUpdate, CommanderPolicy, CommanderProposal, CommanderReview,
  CommanderPlan, CommanderPlanUpdate, CommanderRunRecord,
  CommanderSchedule, CommanderScheduleUpdate, CommanderWorkflow, CommanderWorkflowExecution,
  CommanderWorkflowStep, DecisionAnswer, DecisionRequest,
  DevFlowProjectionState, DispatchDiagnostic, ExecutionAttempt, ExecutionAttemptStatus, ScopeBoundaryHit,
  ExecutionBatch, ExecutionBatchStatus,
  ExecutionRecord, ExecutionStatus, OrchestrationAgent, Phase, PhaseAssignment, RuntimeResultPackage,
  RuntimeSession, RuntimeSessionStatus,
  RuntimeTaskPackage, ResultVerdict, TemporaryAgentStatus,
} from './types.ts'
...
export function upsertDispatchDiagnostic(
  diagnostics: Record<string, DispatchDiagnostic>, diagnostic: DispatchDiagnostic,
): Record<string, DispatchDiagnostic> {
...
export const devflowProjectionSchema: ZodType<DevFlowProjectionState> = zod.object({
  project: zod.union([
    zod.object({
      id: zod.string(),
      name: zod.string(),
  ...
  paused: zod.boolean(),
}) as unknown as ZodType<DevFlowProjectionState>

/** Fold one imported review verdict into a task's consecutive failure count. */
export function foldReviewFailCount(
...
export function resetReviewFailCount(counts: Record<string, number>, taskId: string): Record<string, number> {
  if (counts[taskId] === undefined) return counts
  const { [taskId]: _reset, ...remaining } = counts
  return remaining
}

/** Fold one agent upsert into the instance list (replace by id, else append). */
export function upsertAgent(agents: readonly AgentInstance[], instance: AgentInstance): readonly AgentInstance[] {
...
export function upsertOrchestrationAgent(
...
export function removeOrchestrationAgent(
  agents: Record<string, OrchestrationAgent>,
  agentId: string,
): Record<string, OrchestrationAgent> {
...
export function patchOrchestrationAgent(
  agents: Record<string, OrchestrationAgent>,
  agentId: string,
  ...
  if (existing === undefined) return agents
...
export function transitionOrchestrationAgent(
  agents: Record<string, OrchestrationAgent>,
  agentId: string,
...
export function upsertPhase(phases: Record<string, Phase>, phase: Phase): Record<string, Phase> {
...
export function patchCheckpoint(
  ...
  if (existing === undefined) return checkpoints
...
export function upsertAssignment(
  assignments: Record<string, PhaseAssignment>,
  assignment: PhaseAssignment,
): Record<string, PhaseAssignment> {
 ...
 */
export function patchAssignmentStatus(
  assignments: Record<string, PhaseAssignment>,
  assignmentId: string,
  ...
  if (existing === undefined) return assignments
  const { closedAt: _closedAt, closeReason: _closeReason, ...rest } = existing
  const next: PhaseAssignment = {
    ...rest,
    status,
...
export function upsertExecutionBatch(
  batches: Record<string, ExecutionBatch>,
  batch: ExecutionBatch,
): Record<string, ExecutionBatch> {
  return { ...batches, [batch.batchId]: batch }
}

/** Fold one execution batch status change into the batch map. */
export function transitionExecutionBatch(
  batches: Record<string, ExecutionBatch>,
  batchId: string,
  ...
  if (existing === undefined) return batches
...
export function transitionExecution(
  ...
  if (existing === undefined) return executions
  const { closedAt: _closedAt, closeReason: _closeReason, ...rest } = existing
...
export function upsertExecutionAttempt(
  attempts: Record<string, ExecutionAttempt>,
  attempt: ExecutionAttempt,
): Record<string, ExecutionAttempt> {
...
export function transitionExecutionAttempt(
  ...
  if (existing === undefined) return attempts
...
export function upsertAgentReport(
  reports: Record<string, AgentReport>,
  report: AgentReport,
): Record<string, AgentReport> {
  return { ...reports, [report.reportId]: report }
}

/** Fold one blocked report into the blocked map (replace by id). */
export function upsertBlockedReport(
  blocked: Record<string, BlockedReport>,
  report: BlockedReport,
): Record<string, BlockedReport> {
 ...
 */
export function blockedReportsForTask(
  blocked: Record<string, BlockedReport>,
  taskId: string,
): readonly BlockedReport[] {
...
export function patchAgentReport(
  reports: Record<string, AgentReport>,
  reportId: string,
  ...
  if (existing === undefined) return reports
...
export function upsertCommanderDecision(
  decisions: Record<string, CommanderDecision>,
  decision: CommanderDecision,
): Record<string, CommanderDecision> {
  return { ...decisions, [decision.decisionId]: decision }
}

/** Fold one commander decision patch into the decision map. */
export function patchCommanderDecision(
  decisions: Record<string, CommanderDecision>,
  decisionId: string,
  ...
  if (existing === undefined) return decisions
...
export function upsertCommanderAction(
  actions: Record<string, CommanderAction>,
  action: CommanderAction,
): Record<string, CommanderAction> {
  return { ...actions, [action.actionId]: action }
}

/** Fold one commander control action status change into the action map. */
export function transitionCommanderAction(
  actions: Record<string, CommanderAction>,
  actionId: string,
  ...
  if (existing === undefined) return actions
...
export function upsertCommanderReview(
  reviews: Record<string, CommanderReview>,
  review: CommanderReview,
): Record<string, CommanderReview> {
  return { ...reviews, [review.reviewId]: review }
}

/** Fold one commander review completion (status → reviewed). */
export function completeCommanderReview(
  reviews: Record<string, CommanderReview>,
  reviewId: string,
  at: string,
): Record<string, CommanderReview> {
  const existing = reviews[reviewId]
  if (existing === undefined) return reviews
...
export function upsertRuntimePackage(
  packages: Record<string, RuntimeTaskPackage>,
  pkg: RuntimeTaskPackage,
): Record<string, RuntimeTaskPackage> {
  return { ...packages, [pkg.packageId]: pkg }
}

/** Fold one runtime result package import into the result map (replace by id). */
export function upsertRuntimeResult(
  results: Record<string, RuntimeResultPackage>,
  result: RuntimeResultPackage,
): Record<string, RuntimeResultPackage> {
...
export function upsertCommanderMemory(
  memories: Record<string, CommanderMemory>,
  memory: CommanderMemory,
): Record<string, CommanderMemory> {
  return { ...memories, [memory.memoryId]: memory }
}

/** Fold one commander memory patch into the memory map. */
export function patchCommanderMemory(
  memories: Record<string, CommanderMemory>,
  memoryId: string,
  ...
  if (existing === undefined) return memories
...
export function upsertCommanderSchedule(
  schedules: Record<string, CommanderSchedule>,
  schedule: CommanderSchedule,
): Record<string, CommanderSchedule> {
  return { ...schedules, [schedule.scheduleId]: schedule }
}

/** Fold one commander schedule patch into the schedule map (rolls the next run). */
export function patchCommanderSchedule(
  schedules: Record<string, CommanderSchedule>,
  scheduleId: string,
  ...
  if (existing === undefined) return schedules
...
export function pauseCommanderSchedule(
  schedules: Record<string, CommanderSchedule>,
  scheduleId: string,
  at: string,
): Record<string, CommanderSchedule> {
...
export function completeCommanderRun(
  ...
  if (existing === undefined) return runs
...
export function failCommanderRun(
  runs: Record<string, CommanderRunRecord>,
  runId: string,
  at: string,
): Record<string, CommanderRunRecord> {
...
export function upsertCommanderActionExecution(
  executions: Record<string, CommanderActionExecutionRecord>,
  execution: CommanderActionExecutionRecord,
): Record<string, CommanderActionExecutionRecord> {
  return { ...executions, [execution.executionId]: execution }
}

/** Fold one commander action execution completion (status → completed, success, stamp). */
export function completeCommanderActionExecution(
  executions: Record<string, CommanderActionExecutionRecord>,
  executionId: string,
  at: string,
): Record<string, CommanderActionExecutionRecord> {
...
export function failCommanderActionExecution(
  executions: Record<string, CommanderActionExecutionRecord>,
  executionId: string,
  error: string,
  at: string,
...
export function upsertCommanderExecutionContext(
  contexts: Record<string, CommanderExecutionContext>,
  context: CommanderExecutionContext,
): Record<string, CommanderExecutionContext> {
  return { ...contexts, [context.contextId]: context }
}

/** Fold one commander workflow create/update into the map (replace by id). */
export function upsertCommanderWorkflow(
  workflows: Record<string, CommanderWorkflow>,
  workflow: CommanderWorkflow,
): Record<string, CommanderWorkflow> {
  return { ...workflows, [workflow.workflowId]: workflow }
}

/** Fold one commander workflow start (created → running, stamp). */
export function startCommanderWorkflow(
  workflows: Record<string, CommanderWorkflow>,
  workflowId: string,
  at: string,
): Record<string, CommanderWorkflow> {
...
export function completeCommanderWorkflow(
  workflows: Record<string, CommanderWorkflow>,
  workflowId: string,
  at: string,
): Record<string, CommanderWorkflow> {
...
export function failCommanderWorkflow(
  workflows: Record<string, CommanderWorkflow>,
  workflowId: string,
  at: string,
): Record<string, CommanderWorkflow> {
...
export function appendCommanderWorkflowExecution(
  workflows: Record<string, CommanderWorkflow>,
  workflowId: string,
...
export function upsertDecisionRequest(
  requests: Record<string, DecisionRequest>,
  request: DecisionRequest,
): Record<string, DecisionRequest> {
...
export function answerDecisionRequest(
  ...
  if (request === undefined || request.status !== 'pending') return requests
...
export function appendScopeBoundaryHit(
  hits: readonly ScopeBoundaryHit[],
  hit: ScopeBoundaryHit,
): readonly ScopeBoundaryHit[] {
  return [...hits, hit]
}

/** Fold one commander workflow step create/update into the map (replace by id). */
export function upsertCommanderWorkflowStep(
  steps: Record<string, CommanderWorkflowStep>,
  step: CommanderWorkflowStep,
): Record<string, CommanderWorkflowStep> {
  return { ...steps, [step.stepId]: step }
}

/** Fold one commander workflow step start (pending → running, stamp). */
export function startCommanderWorkflowStep(
  steps: Record<string, CommanderWorkflowStep>,
  stepId: string,
  at: string,
): Record<string, CommanderWorkflowStep> {
...
export function completeCommanderWorkflowStep(
  steps: Record<string, CommanderWorkflowStep>,
  stepId: string,
  at: string,
): Record<string, CommanderWorkflowStep> {
...
export function failCommanderWorkflowStep(
  steps: Record<string, CommanderWorkflowStep>,
  stepId: string,
  at: string,
): Record<string, CommanderWorkflowStep> {
