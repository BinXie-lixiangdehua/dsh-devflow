...
import { dispatchGateMessage, evaluateDispatchGates, makeDispatchDiagnostic } from './dispatch-gates.ts'
import { resolveTaskScope } from './scope-guard.ts'
import type { DevFlowStore } from './storage.ts'
import type { DevFlowSessionStores, DevFlowSessionScope } from './session-store.ts'
...
import { TaskWorkflow, type TaskTransitionResult } from './workflow.ts'
import { AgentWorkflow } from './workflow-agent.ts'
import type { AssignedRole, DecisionAnswer, DecisionOption, DecisionTrigger, DispatchDiagnostic, ExecutionAttempt, ExecutionBatch, ExecutionRecord, PhaseAssignment, PhaseStatus, Project, Result, ScopeGuard, Task, TaskScopeGuard, TaskStatus } from './types.ts'

import { recordDevFlowChange } from './journal.ts'
import { DEFAULT_FIXED_AGENTS, DECLARED_EMPLOYEE_TOOL_NAMES, TEMPORARY_READ_ONLY_TOOLS, WRITE_CLASS_TOOLS, dispatchableToolNames, reconcileFixedAgents } from './default-agents.ts'
import { blockedReportFrom, capabilityBreaker, declaredOutcome } from './blocked-report.ts'
...
function safeReportSummary(value: string): string {
...
export function inFlightDispatchCount(): number {
 ...
 */
export function mayDispatchConcurrently(): boolean {
 ...
 */
function beginInFlightDispatch(): () => void {
...
async function prepareTaskForDispatch(
  workflow: TaskWorkflow,
  store: DevFlowStore,
  taskId: string,
  append: (change: TaskTransitionResult['change'], title: string) => Promise<void>,
): Promise<Task> {
  let task = await store.getTask(taskId)
  if (task === undefined) throw new Error(`devflow: unknown task ${taskId}`)
...
function appendDispatchDiagnostic(store: DevFlowStore, diagnostic: DispatchDiagnostic): void {
  void recordDevFlowChange(store, 'devflow/dispatch/diagnostic', { diagnostic: makeDispatchDiagnostic(diagnostic) }).catch(() => {})
}

function dispatchFailureCode(cause: unknown): string {
...
function dispatchFailureSummary(code: string): string {
...
function safeResultProblems(problems: readonly string[]): string {
  return problems
...
function summaryWithProblems(code: string, problems?: readonly string[]): string {
...
function retryInstruction(problems: readonly string[]): string {
  const detail = safeResultProblems(problems)
  return [
 ...
 */
async function latestReworkReason(store: DevFlowStore, taskId: string): Promise<Result | undefined> {
  ...
  for (let index = imports.length - 1; index >= 0; index -= 1) {
    ...
    const data = entry.data as { readonly taskId?: unknown; readonly resultId?: unknown; readonly verdict?: unknown }
    if (data.taskId !== taskId) continue
    if (data.verdict !== 'changes-requested' || typeof data.resultId !== 'string') return undefined
 ...
 */
async function resolveTaskBounds(store: DevFlowStore, task: Task): Promise<{
  readonly instructions: string
  readonly acceptanceCriteria: readonly string[]
  readonly scopeGuard: TaskScopeGuard | undefined
  readonly scope: ScopeGuard | undefined
  readonly source: 'task' | 'project' | 'none'
}> {
  ...
  const source: 'task' | 'project' | 'none' = taskScope !== undefined ? 'task' : (projectScope !== undefined ? 'project' : 'none')
...
function fileNamesOf(text: string): Set<string> {
...
const DECISION_TRIGGERS: readonly DecisionTrigger[] = [
 ...
 * inline.
 */
export const DECISION_CUSTOM_OPTION: DecisionOption = {
...
function decisionArgsError(detail: string): Error {
 ...
 */
function parseDecisionField(value: unknown, field: string): unknown {
 ...
 */
function assertDecisionOptionRules(options: readonly DecisionOption[], recommended: string): void {
 ...
 */
export function parseDecisionRequestArgs(raw: unknown): DecisionRequestArgs {
  ...
  if (typeof trigger !== 'string' || !DECISION_TRIGGERS.includes(trigger as DecisionTrigger)) {
  ...
  return {
    ...
    trigger: trigger as DecisionTrigger,
    question,
 ...
 */
function assertPackageScopeConsistency(task: Task, scope: ScopeGuard | undefined): void {
...
interface DirectDispatchRuntime {
  batch?: ExecutionBatch
  execution?: ExecutionRecord
  attempt?: ExecutionAttempt
}

async function completeBatch(store: DevFlowStore, runtime: DirectDispatchRuntime): Promise<void> {
  if (runtime.batch === undefined) return
  let batch = await store.getBatch(runtime.batch.batchId)
  if (batch === undefined || batch.status === 'completed') return
...
async function failDirectDispatch(
  store: DevFlowStore,
  runtime: DirectDispatchRuntime,
  agentId: string,
  code: string,
  outcome: { readonly reportStatus?: 'failed' | 'blocked'; readonly problems?: readonly string[] } = {},
): Promise<void> {
  if (runtime.attempt !== undefined) {
    ...
    if (attempt !== undefined) runtime.attempt = attempt
  }
 ...
 */
export function effectiveTemporaryTools(
  declared: readonly string[] | undefined,
  callerView: ReadonlySet<string> = new Set(),
): readonly string[] {
  if (declared === undefined || declared.length === 0) return [...TEMPORARY_READ_ONLY_TOOLS]
  const allowed = new Set<string>([...DECLARED_EMPLOYEE_TOOL_NAMES, ...callerView])
  const trimmed: string[] = []
  for (const name of declared) {
    if (allowed.has(name) && !trimmed.includes(name)) trimmed.push(name)
  }
  return trimmed
}

/** Whether one tool list declares a writing capability (the auditable half of the rule). */
export function declaresWriteClassTools(tools: readonly string[]): readonly string[] {
...
function inheritedModelConfig(role: AssignedRole): { readonly model: string } {
...
function temporaryAgentPrompt(instance: AgentInstance): string {
 ...
 */
export function authorizeAgentRegistration(
  isCommander: ((agent: Agent) => boolean) | undefined,
  ...
  if (isCommander === undefined || !isCommander(caller)) {
    throw new Error(
      `devflow: registering an employee is restricted to the Commander; caller ${caller.id} is not authorized`,
    )
  }
  return caller.id
}
 ...
 * @param isCommander - the Commander-seat check supplied by the composition.
 */
async function registerDispatchableEmployee(
  store: DevFlowStore,
  instance: AgentInstance,
  caller: Agent | undefined,
  isCommander: ((agent: Agent) => boolean) | undefined,
  declaredTools?: readonly string[],
): Promise<{ readonly agent: OrchestrationAgent; readonly tools: readonly string[]; readonly writeClass: readonly string[] }> {
  ...
  const registered: OrchestrationAgent = existing === undefined
 ...
 */
const workflowByStore = new WeakMap<DevFlowStore, { readonly workflow: TaskWorkflow; readonly agentWorkflow: AgentWorkflow }>()
...
function bindSession(
  services: DevFlowToolServices,
  agent: Agent | undefined,
): { readonly scope: DevFlowSessionScope; readonly store: DevFlowStore; readonly workflow: TaskWorkflow; readonly agentWorkflow: AgentWorkflow } {
  ...
  if (scope.store === store) return { scope, store, workflow, agentWorkflow }
 ...
 */
export function registerDevFlowTools(ctx: Context, services: DevFlowToolServices): void {
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      const { store: sessionStore, workflow: sessionWorkflow } = bindSession(services, exec.agent)
      let result: TaskTransitionResult
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      if (task === undefined) throw new Error(`devflow: unknown task ${args.taskId}`)
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      const exported = await sessionAgentWorkflow.exportTask(args.taskId, updated => (
        recordDevFlowChange(sessionStore, 'devflow/project/update', { project: updated }).then(() => undefined)
      ))
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      if (args.taskId === undefined) {
        ...
        return { cleared: 'project-default' as const }
      }
      ...
      return { cleared: 'task' as const, taskId: args.taskId }
    },
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      if (await sessionStore.getAgent(args.agentId) === undefined) throw new Error(`devflow: unknown orchestration agent ${args.agentId}`)
      if (await sessionStore.getTask(args.taskId) === undefined) throw new Error(`devflow: unknown task ${args.taskId}`)
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (rawArgs, exec) => {
      ...
      if (project === undefined) throw new Error('devflow: no project initialized before requesting a decision')
      ...
      if (args.taskId !== undefined) {
        ...
        if (blocking.length > 0) {
          ...
          throw new Error(
            `devflow: task ${args.taskId} is blocked by a reported capability gap (${newest.gapKind}: ${newest.missing}); `
      ...
      const request = {
        ...
        allowCustom: true, status: 'pending' as const, answer: null, createdAt: now, answeredAt: null,
      }
      ...
      if (userQuestions === undefined) throw new Error('devflow: user questions service unavailable')
      ...
      if (first === undefined) throw new Error('devflow: user decision returned no answer')
      ...
      const answer: DecisionAnswer = first.custom === undefined
      ...
      return { requestId: request.requestId, answer: answer as Record<string, JsonValue> }
    },
  ...
  ctx.inject(['subagents'], (dispatchCtx) => {
    ...
    execute: async (args, exec) => {
      ...
      try {
        ...
        if (parent === undefined) throw new Error('devflow: dispatch requires a calling commander agent')
      ...
      if (child === undefined) throw new Error(`devflow: unknown orchestration agent ${args.agentId}`)
      const initialTask = await store.getTask(args.taskId)
      if (initialTask === undefined) throw new Error(`devflow: unknown task ${args.taskId}`)
      let task: Task = initialTask
      const dispatchId = String(exec.callId)
      const projectBefore = await store.loadProject()
      if (projectBefore === undefined) throw new Error('devflow: no project initialized before dispatch')
      ...
      let activeAssignment: PhaseAssignment | undefined
      const runtime: DirectDispatchRuntime = {}
      /** Child-reply attempts spent and the last bounded document problems. */
      let resultAttempts = 0
      let resultProblems: readonly string[] = []
      /**
      ...
      try {
        ...
        assertPackageScopeConsistency(task, bounds.scope)
        task = await prepareTaskForDispatch(workflow, store, task.id, (change, title) => (
          recordDevFlowChange(store, 'devflow/task/transition', { ...change, title }).then(() => undefined)
        ))
        ...
        const prepared = await prepareTaskPackage(store, task.id, {
          ...
          recordDevFlowChange(store, 'devflow/project/update', { project: updated }).then(() => undefined)
        ))
        ...
        const startChild = async (promptText: string) => {
          const run = await dispatchCtx.subagents.start('spawn', {
            label: args.description ?? `DevFlow ${child.role} task`,
            prompt: [{ type: 'text', text: promptText } satisfies ContentBlock],
            parent,
            signal: exec.signal,
            persona,
        ...
        let parsed: ReturnType<MarkdownBridge['importResult']> | undefined
        let lastRunId = ''
        let stopReason = ''
        ...
        if (runtime.attempt === undefined || runtime.execution === undefined) throw new Error('devflow: direct dispatch runtime was not initialized')
        ...
        await failDirectDispatch(store, runtime, child.agentId, code, {
          ...(code === 'DEVFLOW_DISPATCH_RESULT_REJECTED' ? { reportStatus: 'blocked' as const } : {}),
        ...
        const failureDiagnostic = {
          ...
          projectionStatus: gate.projectionStatus, reviewFailCount: gate.reviewFailCount, decisionStatus: gate.decisionStatus,
          highRisk: gate.highRisk, ...(activeAssignmentId === undefined || activeAssignment === undefined ? {} : { assignmentId: activeAssignmentId, assignmentAgentId: activeAssignment.agentId, assignmentPhaseId: activeAssignment.phaseId, assignmentStatus: 'assigned' as const }),
          ...
          status: 'failed' as const, at: new Date().toISOString(),
        }
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      return {
        ...
        ...(metadata === undefined ? {} : { metadata: metadata as unknown as Record<string, JsonValue> }),
      }
    },
