...
export const COMMANDER_READ_ONLY_TOOLS: readonly string[] = ['read', 'glob', 'grep']
 ...
 */
const EMPLOYEE_FILE_TOOLS: readonly string[] = ['read', 'write', 'edit', 'glob', 'grep', 'read_image', 'pwsh']

/** The four default fixed employees and their approved Skill bindings. */
export const DEFAULT_FIXED_AGENTS: readonly DefaultAgentInput[] = [
  {
 ...
 */
export const DECLARED_EMPLOYEE_TOOL_NAMES: ReadonlySet<string> = new Set(
  DEFAULT_FIXED_AGENTS.flatMap(agent => [...agent.tools]),
)
 ...
 */
export const TEMPORARY_READ_ONLY_TOOLS: readonly string[] = ['read', 'glob', 'grep', 'read_image']
 ...
 */
export const WRITE_CLASS_TOOLS: ReadonlySet<string> = new Set(['write', 'edit', 'pwsh', 'str_replace_editor'])
 ...
 */
export function dispatchableToolNames(
  declared: readonly string[],
  parentView: ReadonlySet<string>,
  known: ReadonlySet<string> = DECLARED_EMPLOYEE_TOOL_NAMES,
): readonly string[] {
...
function configOf(input: DefaultAgentInput) {
  return {
    role: input.role,
    prompt: input.prompt,
...
function drifted(stored: OrchestrationAgent, input: DefaultAgentInput): boolean {
 ...
 */
export async function reconcileFixedAgents(store: DevFlowStore): Promise<FixedAgentReconciliation> {
  const added: string[] = []
 ...
 * @returns the ids added and the ids refreshed.
 */
export async function recordFixedRosterChanges(store: DevFlowStore): Promise<FixedAgentReconciliation> {
  ...
  for (const agentId of [...reconciliation.added, ...reconciliation.refreshed]) {
    ...
    if (agent !== undefined) await recordDevFlowChange(store, 'devflow/agent/register', { agent })
  }
