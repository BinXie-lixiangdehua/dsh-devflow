...
export type DecisionAnswer = { readonly optionId: string } | { readonly custom: string }
...
export const DEVFLOW_CLOSE_REASONS: readonly DevFlowCloseReason[] = ['stale-lost', 'superseded', 'abandoned']

/** Is this value one of the three close reasons? */
export function isDevFlowCloseReason(value: unknown): value is DevFlowCloseReason {
  return typeof value === 'string' && (DEVFLOW_CLOSE_REASONS as readonly string[]).includes(value)
}
...
export type DevFlowActivationCode =
  | 'devflow-preset-identity-mismatch'
  | 'devflow-session-mismatch'
  | 'devflow-project-unavailable'
  | 'devflow-project-init-failed'
  | 'devflow-commander-install-failed'
  | 'devflow-activation-verification-failed'
  | 'devflow-commander-not-installed'
  | 'devflow-deactivation-failed'
  | 'devflow-restore-failed'
  | 'devflow-journal-failed'
  | 'devflow-host-unavailable'
...
export interface DevFlowJournalEventMap {
     ...
     * no project.
     */
    'devflow/project/update': { project: Project }
    /**
     ...
     */
    'devflow/task/transition': { taskId: string; title?: string; from: TaskStatus | null; to: TaskStatus; at: string }
    /**
     * Whole-value agent instance snapshot, appended on every instance
     * create/update. Last one wins per instance id.
     */
    'devflow/agent/upsert': { instance: AgentInstance }
    /**
     * One task export record: a handoff document was produced for a task.
     * Log-only, non-surface.
     */
    'devflow/bridge/export': { taskId: string; bridge: string; at: string }
    /**
     ...
     */
    'devflow/bridge/import': { taskId: string; resultId: string; protocolVersion: string; verdict: ResultVerdict; at: string }
    /**
     * One planner resume generation record. Facts only — no model output,
     * no planning behavior.
     */
    'devflow/planner/resume': { taskId: string; at: string }
    /**
     * One orchestration agent registered (fixed → active, temporary → created).
     */
    'devflow/agent/register': { agent: OrchestrationAgent }
    /** One orchestration agent removed from the registry. */
    'devflow/agent/remove': { agentId: string; at: string }
    /** One orchestration agent config patch applied. */
    'devflow/agent/update-config': { agentId: string; patch: AgentConfigPatch; at: string }
    /** One temporary-agent lifecycle transition (created → running → terminated). */
    'devflow/agent/transition': { agentId: string; from: TemporaryAgentStatus; to: TemporaryAgentStatus; at: string }
    /** The MVP plan was created (or replaced). */
    'devflow/mvp/create': { plan: MvpPlan }
    /** One phase was created. */
    'devflow/phase/create': { phase: Phase }
    /** One phase status change. */
    'devflow/phase/update': { phaseId: string; status: PhaseStatus; at: string }
    /** The PROJECT default scope guard was updated (fallback for unscoped tasks). */
    'devflow/scope/update': { scope: ScopeGuard }
    /** The project default scope guard was cleared. */
    'devflow/scope/clear': { at: string }
    /**
     ...
     */
    'devflow/scope/task-update': { taskId: string; scope: ScopeGuard }
    /** One task's own scope guard was cleared. */
    'devflow/scope/task-clear': { taskId: string; at: string }
    /** One task execution reached a ScopeGuard limit or completion criterion. */
    'devflow/scope/boundary-hit': { hit: ScopeBoundaryHit }
    /** One Commander decision request was created. */
    'devflow/decision/request': { request: DecisionRequest }
    /** One pending Commander decision request was answered. */
    'devflow/decision/answer': { requestId: string; answer: DecisionAnswer; at: string }
    /**
    ...
    'devflow/commander/mode-enter': { projectId: string; sessionId: string; at: string }
    /** Interactive Commander mode returned to native chat. */
    'devflow/commander/mode-exit': { at: string }
    /** New task dispatches pause after current work settles. */
    'devflow/control/pause': { at: string }
    /** New task dispatches resume. */
    'devflow/control/resume': { at: string }
    /** One out-of-scope improvement was queued. */
    'devflow/improvement/add': { improvement: Improvement }
    /** One commander checkpoint was created. */
    'devflow/commander/checkpoint/create': { checkpoint: CommanderCheckpoint }
    /** One commander checkpoint was patched. */
    'devflow/commander/checkpoint/update': { checkpointId: string; patch: CommanderCheckpointUpdate; at: string }
    /** One phase → agent assignment was created. */
    'devflow/orchestration/assign': { assignment: PhaseAssignment }
    /** One task dispatch diagnostic snapshot. */
    'devflow/dispatch/diagnostic': { diagnostic: DispatchDiagnostic }
    /** One phase → agent assignment was removed. */
    'devflow/orchestration/unassign': { assignmentId: string; at: string }
    /** One phase → agent assignment status changed. */
    'devflow/orchestration/update': { assignmentId: string; status: AssignmentStatus; at: string; closeReason?: DevFlowCloseReason }
    /** One stale assignment was closed (terminal, never dispatched again). */
    'devflow/orchestration/close': { assignmentId: string; closeReason: DevFlowCloseReason; at: string }
    /** One commander plan was created. */
    'devflow/commander/plan/create': { plan: CommanderPlan }
    /** One commander plan was patched. */
    'devflow/commander/plan/update': { planningId: string; patch: CommanderPlanUpdate; at: string }
    /** One commander plan was activated. */
    'devflow/commander/plan/activate': { planningId: string; at: string }
    /** One execution batch was created. */
    'devflow/execution/batch/create': { batch: ExecutionBatch }
    /** One execution batch was started. */
    'devflow/execution/batch/start': { batchId: string; at: string }
    /** One execution batch was paused. */
    'devflow/execution/batch/pause': { batchId: string; at: string }
    /** One execution batch was completed. */
    'devflow/execution/batch/complete': { batchId: string; at: string }
    /** One execution started (record born running). */
    'devflow/execution/start': { execution: ExecutionRecord }
    /** One execution status changed. */
    'devflow/execution/update': { executionId: string; status: ExecutionStatus; at: string; closeReason?: DevFlowCloseReason }
    /** One execution completed. */
    'devflow/execution/complete': { executionId: string; at: string }
    /** One execution failed. */
    'devflow/execution/fail': { executionId: string; at: string }
    /** One stale execution was closed (terminal, never runs again). */
    'devflow/execution/close': { executionId: string; closeReason: DevFlowCloseReason; at: string }
    /** One execution attempt was created (status `created`). */
    'devflow/execution/attempt/create': { attempt: ExecutionAttempt }
    /** One execution attempt began running (created → running). */
    'devflow/execution/attempt/start': { attemptId: string; at: string }
    /** One execution attempt completed (running → completed). */
    'devflow/execution/attempt/complete': { attemptId: string; at: string }
    /** One execution attempt failed (running → failed). */
    'devflow/execution/attempt/fail': { attemptId: string; at: string }
    /** One agent report was created. */
    'devflow/agent/report/create': { report: AgentReport }
    /** One agent report was patched. */
    'devflow/agent/report/update': { reportId: string; patch: AgentReportUpdate; at: string }
    /** One employee reported it was blocked, with the structured reason. */
    'devflow/blocked/report': { blocked: BlockedReport }
    /** One commander decision was created. */
    'devflow/commander/decision/create': { decision: CommanderDecision }
    /** One commander decision was patched. */
    'devflow/commander/decision/update': { decisionId: string; patch: CommanderDecisionUpdate; at: string }
    /** One runtime task package was exported. */
    'devflow/runtime/export': { package: RuntimeTaskPackage }
    /** One runtime result package was imported. */
    'devflow/runtime/import': { result: RuntimeResultPackage }
    /** One runtime session was created (status `created`). */
    'devflow/runtime/session/create': { session: RuntimeSession }
    /** One runtime session began running (created → running). */
    'devflow/runtime/session/start': { sessionId: string; at: string }
    /** One runtime session completed (running → completed). */
    'devflow/runtime/session/complete': { sessionId: string; at: string }
    /** One runtime session failed (running → failed). */
    'devflow/runtime/session/fail': { sessionId: string; at: string }
    /** One commander control action was created. */
    'devflow/commander/action/create': { action: CommanderAction }
    /** One commander control action began executing. */
    'devflow/commander/action/execute': { actionId: string; at: string }
    /** One commander control action completed. */
    'devflow/commander/action/complete': { actionId: string; at: string }
    /** One commander review was created (status `pending`). */
    'devflow/commander/review/create': { review: CommanderReview }
    /** One commander review completed (pending → reviewed). */
    'devflow/commander/review/complete': { reviewId: string; at: string }
    /** One structured commander memory was created. */
    'devflow/memory/create': { memory: CommanderMemory }
    /** One structured commander memory was patched. */
    'devflow/memory/update': { memoryId: string; patch: CommanderMemoryUpdate; at: string }
    /** One commander cycle schedule was created (status `active`). */
    'devflow/commander/schedule/create': { schedule: CommanderSchedule }
    /** One commander cycle schedule was patched (interval or run stamps). */
    'devflow/commander/schedule/update': { scheduleId: string; patch: CommanderScheduleUpdate; at: string }
    /** One commander cycle schedule was paused (active → paused). */
    'devflow/commander/schedule/pause': { scheduleId: string; at: string }
    /** One commander runtime driver run was created (status `running`). */
    'devflow/commander/run/create': { run: CommanderRunRecord }
    /** One commander runtime driver run completed (running → completed). */
    'devflow/commander/run/complete': { runId: string; decisionId: string; actionId: string; at: string }
    /** One commander runtime driver run failed (running → failed). */
    'devflow/commander/run/fail': { runId: string; at: string }
    /** One commander action execution was created (status `running`). */
    'devflow/commander/action-execution/create': { execution: CommanderActionExecutionRecord }
    /** One commander action execution completed (running → completed). */
    'devflow/commander/action-execution/complete': { executionId: string; at: string }
    /** One commander action execution failed (running → failed). */
    'devflow/commander/action-execution/fail': { executionId: string; error: string; at: string }
    /** The project governance policy was created or replaced (last one wins). */
    'devflow/policy/update': { policy: CommanderPolicy }
    /** One commander proposal was created (status `created`). */
    'devflow/commander/proposal/create': { proposal: CommanderProposal }
    /** One commander proposal was approved (created → approved). */
    'devflow/commander/proposal/approve': { proposalId: string; at: string }
    /** One commander proposal was rejected (created → rejected). */
    'devflow/commander/proposal/reject': { proposalId: string; at: string }
    /** One immutable commander runtime execution context was generated. */
    'devflow/commander/execution-context/create': { context: CommanderExecutionContext }
    /** One commander development workflow was created (status `created`). */
    'devflow/commander/workflow/create': { workflow: CommanderWorkflow }
    /** One commander development workflow started (created → running). */
    'devflow/commander/workflow/start': { workflowId: string; at: string }
    /** One commander development workflow completed (running → completed). */
    'devflow/commander/workflow/complete': { workflowId: string; at: string }
    /** One commander development workflow failed (running → failed). */
    'devflow/commander/workflow/fail': { workflowId: string; at: string }
    /** One execution-history entry was appended to a workflow. */
    'devflow/commander/workflow/execution/add': { workflowId: string; entry: CommanderWorkflowExecution; at: string }
    /** One commander workflow step was created (status `pending`). */
    'devflow/commander/step/create': { step: CommanderWorkflowStep }
    /** One commander workflow step started (pending → running). */
    'devflow/commander/step/start': { stepId: string; at: string }
    /** One commander workflow step completed (running → completed). */
    'devflow/commander/step/complete': { stepId: string; at: string }
    /** One commander workflow step failed (running → failed). */
    'devflow/commander/step/fail': { stepId: string; at: string }
}
...
export type DevFlowJournalEvent<T extends DevFlowJournalEventType = DevFlowJournalEventType> = {
  [K in T]: { readonly type: K; readonly seq: number; readonly time: number; readonly data: DevFlowJournalEventMap[K] }
}[T]
